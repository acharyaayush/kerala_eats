...
class Admin extends CI_Controller {
   ...
   function __construct(){
	...
	public function index() {
		// echo base_url();die;
		// echo base_url('admin/user_management');die;
		/* if($this->id){
		 	$data = array(
	...
	public function admin_dashboard() {
		...
		if($this->id){
			...
			$pageData['total_restaurant_admin'] = $this->Common->getData('users','count(id) as total_restaurant_admin','role = 2');// Role 2 = Restaurant admin
			...
			$pageData['total_order'] = $this->Common->getData('orders','count(id) as total_order');
		...
		if($this->id != '' && $this->role == 1 || $this->role == 2)
        {
        	 ...
        	 //pagination ---------------------START-------------------
			   ...
			   //customer detail page for show ing customer order details----
				...
				$pageData['orders_data'] = $this->Common->getData('orders','orders.*,users.number_id as user_number_id,users.fullname as customer_name,users.email,restaurants.rest_name,restaurants.delivery_time,rest_accept_types.name as order_type_name,merchant_categories.category_name as business_category_name','orders.order_status != 9 '.$query_part .' '.$query_customer_order_detail.' ',array('users','restaurants','rest_accept_types','merchant_categories'),array('orders.user_id = users.id','orders.restaurant_id = restaurants.id','orders.order_type = rest_accept_types.id','orders.business_category = merchant_categories.id'),'orders.id','DESC', $pagination_query_part, $page_offset);//order status - 	//  0 -Placed and Pending 1 - Accepted 2 - Rejected 3 - Dispatched 4 - Cancelled 5 - Completed 6 Preparing 7 Ready, 9 -delete
	...
	public function Export_Order_CSV( $fromdate='all',$todate='all' ,$delivery_handle_by='all',$payment_mode='all',$paid_status='all',$is_paid_to_restaurant='all',$order_status='all',$search_restaurant_id='all',$search_customer_id='all',$order_accept_type='all',$business_category_id='all',$is_cutlery_needed='all',$is_promocode_auto_applied='all',$is_promocode_auto_applied_on_delivery='all',$search_key='all'){
	   ...
	   if($fromdate!='all' || $todate !='all'  || $delivery_handle_by !='all' || $payment_mode!='all' || $paid_status!='all' || $is_paid_to_restaurant!='all' || $order_status!='all' || $search_restaurant_id!='all' || $search_customer_id!='all' || $order_accept_type!='all' || $business_category_id!='all' || $is_cutlery_needed!='all' || $is_promocode_auto_applied!='all' || $is_promocode_auto_applied_on_delivery!='all' || $search_key !='all'){
	         ...
	         if($search_key != "all")
	        {
			 ...
			 $OrderData =  $this->Common->getData('orders','orders.*,users.number_id as user_number_id,users.fullname as customer_name,users.email,restaurants.rest_name,rest_accept_types.name as order_type_name,merchant_categories.category_name as business_category_name','orders.order_status != 9'.$query_part .' ',array('users','restaurants','rest_accept_types','merchant_categories'),array('orders.user_id = users.id','orders.restaurant_id = restaurants.id','orders.order_type = rest_accept_types.id','orders.business_category = merchant_categories.id'),'orders.id','DESC');
			 ...
			 $file = fopen('php://output', 'w');
			  ...
			  if(!$this->logged_in_restaurant_id && $this->logged_in_restaurant_id == "" && $this->role == 1){// only super admin can see and update
			   ...
			   $header = array("Order ID","Ordered Date","Customer Name and ID","Restaurant Name","Delivery Name","Delivery Email","Delivery Mobile No.","Order Status","Cancelled By","Delivery Handled by","Is Cutlery Needed","Payment Mode","Is Promo Code Auto Applied","Is Auto Applied Promo Code Apply on Delivery","Address","Total Amount","".$is_paid_to_restaurant_column .""); 
			  ...
			  fputcsv($file, $header);
			   if(count($OrderData) > 0){
				   foreach ($OrderData as $key=>$line){ 
		               ...
		               $data_array = array($line['order_number'],$createdAt,$line['customer_name'].' ('.$line['user_number_id'].')',$line['rest_name'],$line['delivery_name'],$line['delivery_email'],$line['delivery_mobile'],$status,$cancelled_by,$delivery_handled,$is_cutlery_needed,$payment_mode,$promo_code_is_applied_status,$promo_code_is_applied_on_delivery_status,$line['delivery_address'],$line['total_amount'],$is_paid_to_restaurant_status_value);
	  ...
	  public function add_variant_submit(){
			...
			if($variant_name != "" && !empty($variant_name)){
				$insert_array = [
		                ...
		                'admin_id' => $this->id,// both id will go of admin or merchant
		                'restaurant_id'  => $restaurant_id,
		                'added_by'=> $this->role,//	1 - by super admin, 2 - Merchant/Restaurant
		                'created_at' => time(),
		                'updated_at' => time()
		            ];
  ...
  //Add variant type  --------------END----------------------
    ...
    public function add_variant_for_product(){
        ...
        if($add_edit_mode == 1){// edit mode
        	...
        	foreach ($exists_product_status as  $value) {
        		if(in_array($value['variant_type_id'], $new_select_variant_type_array) ==false){
        			...
        			$update_array = [
				                  'status'=> 2, // 1 - selected , 2 - unselected  (default its 2) // if status is 2 then it will be do 1 and same if 1  then it will be 2
        ...
        if($selected_variant_id != "" && $selected_product_id !="" && ($default_variant_type_id !="" || $default_variant_type_id == "") && ($variant_select_type!="" || $variant_select_type =="") && !empty($array_of_variant_type)){//variant_select_type bu default it will be 1
        		...
        		foreach ($array_of_variant_type as  $value) {
        			...
        			$exists_product_status = $this->Common->getData('variant_types_for_products','status','product_id = '.$selected_product_id.' AND variant_id = '.$selected_variant_id.' AND variant_type_id = '.$value['variant_type_id'].'','','','variant_type_product_id','DESC');
        			   ...
        			   if(!empty($exists_product_status)){
				               ...
				               $update_array = [
				                  'status'=> 1, // 1 - selected , 2 - unselected  (default its 2) // if status is 2 then it will be do 1 and same if 1  then it will be 2
				                  'variant_type_price'=> $value['variant_type_price'], 
				                  'single_select'=> $variant_select_type, //1 - single select 2 multi select
        		...
        		$update_deafult_value = [
                  'default_variant_status'=> $default_variant_status, //default_variant_status = if 1 then this variant type will be set as default for product. and only one variant type 	echo 'dfdf';will be set as default for product
                  'updated_at' => time()
              ];
        	  ...
        	  //update Multi SELECT limit -------START
        		...
        		$update_select_limit_value = [
	                  'multi_select_limit'=>  $variant_select_limit,
	                  'updated_at' => time()
	              ];
		    	 ...
		    	 if($edit_vairant_name == $check_name_is_changed[0]['variant_name']){
		    	 	 echo 4;// nothing changed
			    	 	...
			    	 	if($edit_vairant_name != "" && !empty($edit_vairant_name)){
							...
							$update_array = [
					                'variant_name'=> $edit_vairant_name,
					                'updated_at' => time()
					            ];
		    	 ...
		    	 if($edit_vairant_type_name == $check_name_is_changed[0]['variant_type_name']){
		    	 	 echo 4;// nothing changed
			    	 	...
			    	 	if($edit_vairant_type_name != "" && !empty($edit_vairant_type_name)){
							...
							$update_array = [
					                'variant_type_name'=> $edit_vairant_type_name,
					                'updated_at' => time()
					            ];
				...
				$order_number_id = $user_id[0]['order_number'];
				  if($update_status > 0){
                    ...
                    $notification_data_fields = array(
                        'message' => 'Order '.$order_number_id.' status changed to '.$status,
	                ...
	                $insertData = [
	                	'title' => 'Order '.$order_number_id.' status changed to '.$status,
	                	'to_user_id' => $user_id[0]['user_id'],
	                	'type' => 1, # 1 for order related
	                	'order_id' => $order_id,
	                	'is_read' => 1,
	                	'created_at' => time(),
	                	'updated_at' => time(),
		            		...
		            		if($order_type == 1) # Order now so dont pass scheduleAt
		            		{
		            			...
		            			$body = array(
								  ...
								  "stops" => array(
								    ...
								    array(
								      "location" => array("lat" => $delivery_latitude, "lng" => $delivery_longitude),
								      "addresses" => array(
								        "en_SG" => array(
								          "displayString" => $delivery_address.", ".$unit_number.", ".$postal_code,
								          "country" => "SG" // Country code must follow the country you are at
								        )   
								      )   
								    )   
                    ...
                    if($order_status == 5) # COMPLETED
                    {
		        		...
		        		$insert_cashback = [
		    				'user_id' => $cashback_user_id,
		    				'order_id' => $order_id,
		    				'wallet_date' => time(),
		    				'debited' => 0,
		    				'credited' => $cashback_val,
		    				'type' => 1,
		    				'added_by' => 1, # as it is a cashback on order that means added by admin (kerala eats)
		    			...
		    			$notification_data_fields = array(
	                        'message' => 'Received Cashback of S$'.$cashback_val.' for order '.$order_number_id,
	                        'title' => NOTIFICATION_CASHBACK_RECVD,
	                        'notification_type' => 'ORDER_CASHBACK'
	                    );
	                    ...
	                    # Now insert notification to Database FOR MERCHANT ID
		                $insertData = [
		                	'title' => 'Received Cashback of S$'.$cashback_val.' for order '.$order_number_id,
		                	'to_user_id' => $cashback_user_id,
		                	'type' => 1, # 1 for order related
		                	'order_id' => $order_id,
		                	'is_read' => 1,
		                	'created_at' => time(),
   ...
   public function delete_order($selected_role = ""){
	    ...
	    $update_array = [
	                    'order_status'=>9,  //  0 -Placed and Pending 1 - Accepted 2 - Rejected 3 - Dispatched 4 - Cancelled 5 - Completed 6 Preparing 7 Ready, 9 -delete
	                    'updated_at' => time()
	                ];
	      # update data in orders table
	      $update_status =  $this->Common->updateData('orders',$update_array,"id = ".$order_id."");
...
// change status of is_paid_to_restaurant  -------- START----------
   public function is_paid_to_restaurant_status_update($selected_role = ""){
	    ...
	    if($order_id != ""){
	    	 $update_array = [
	                    'is_paid_to_restaurant'=>$is_paid_to_restaurant_status_value,  // //For is paid to restauant "Indicates whether admin paid to restaurant manually. 1 - Yes 2 -No(means pending)"
	                    'updated_at' => time()
	                ];
		      # update data in orders table
		      $update_status =  $this->Common->updateData('orders',$update_array,"id = ".$order_id."");
...
// change status of is_paid_to_restaurant  -------- END----------
	 ...
	 public function comman_show_orderd_products_function($order_id = "", $selected_product_id = "",$load_mode = "") {  
	 	...
	 	// when  admin will customize orderd data means at the time of load mode 2 , will get selected_product_id value other wise it will be blank
	 	 ...
	 	 // getting Ordered Products -------------START------------
			$order_product_details = $this->Common->getData('order_product_details','order_product_details.order_id,order_product_details.product_id,order_product_details.product_quantity,order_product_details.product_unit_price,products.product_name,products.product_image,products.category_id','order_id = "'.$order_id.'" AND status != 1 '.$query_part.'',array('products'),array('products.id = order_product_details.product_id'));// '.$query_part.' //status = 1- delete , default - 0
           ...
           $product_detail_array = $order_product_details;
        		...
        		if(!empty($product_detail_array))
        		{
        			...
        			{
		        		$order_product_varirant_query = "SELECT variants.variant_name,variant_types.variant_type_name,variant_types.variant_type_id,variant_types.variant_id,order_product_variant_details.variant_price
	...
	public function order_single($order_id = "") {
		...
		if($this->id && ($this->role == 1 || $this->role == 2)){
			...
			$pageData['product_data'] = $this->Common->getData('products','id as product_id,	product_name,price','product_status = 1 AND restaurant_id = '.$restaurant_id.'','','','id','DESC');
	    ...
	    if($order_type != 2 && $delivery_handled_by == 2){//delivery_handled_by by kerala eatss
		    ...
		    if($order_type == 1) # Order now so dont pass scheduleAt
	    			...
	    			$body = array(
							  ...
							  "stops" => array(
							    array(
							      ...
							      "addresses" => array(
							        "en_SG" => array(
							          "displayString" => $lalamove_rest_details[0]['rest_pin_address'],
							          "country" => "SG" // Country code must follow the country you are at
							        )   
							      )   
							    ),  
							  ...
							  "deliveries" => array(
							    array(
							     ...
							     "remarks" => "1. Kerala Eats Food Order ID: [".$order_number_id."] \n2. Customer Name: ".$delivery_name." \n3. Support Number: ".$lalamove_support_number."\n4. Tips pay by Kerala Eats"
							    )   
							  )   
							);
					// echo "<pre> 3020";
		        ...
		        if($lalamove_order_response['failed_reason'] == '')
	            	...
	            	$update_array = [
		                'delivery_address'=> $pin_address,
		                'delivery_postal_code'=> $delivery_postal_code,
		                'delivery_unit_number'=> $delivery_unit_number,
		                'delivery_latitude'=> $latitude,
		                'delivery_longitude'=> $longitude,
		                'checkout_status_by_admin'=> 1,
		                'admin_checkout_delivery_charge_if_change'=> $lalamove_order_amount,
		                'updated_at' => time()
		            ];
					  # update data in orders table
		    ...
		    // calcilate disatance
			 ...
			 $update_array = [
		                'delivery_street_address'=> $pin_address,
		                'delivery_postal_code'=> $delivery_postal_code,
		                'delivery_unit_number'=> $delivery_unit_number,
		                'delivery_latitude'=> $latitude,
		                'delivery_longitude'=> $longitude,
		                'checkout_status_by_admin'=> 1,// for checkout modal show for update delivery amount with item calculation 
		                'admin_checkout_delivery_charge_if_change'=> $rest_delivery_charge,
		                'updated_at' => time()
		            ];
			  # update data in orders table
			  $update_status =  $this->Common->updateData('orders',$update_array,"id = ".$selected_order_id);
	...
	public function update_order_preparation_time_after_accept(){
		...
		$update_array = [
		                'preparation_time_when_accepted'=> $edit_order_preparation_time, //Ex 30 so 30 minutes
		                 'updated_at' => time()
		            ];
		  # update data in orders table
		  $update_status =  $this->Common->updateData('orders',$update_array,"id = ".$selected_order_id);
    ...
    //EDIT Order update _pick_up Time -------------------START------------------
	...
	public function update_order_schedule_time(){
		...
		$update_array = [
		                'schedule_time'=> $order_schedule_time, 
		                 'updated_at' => time()
		            ];
		  # update data in orders table
		  $update_status =  $this->Common->updateData('orders',$update_array,"id = ".$selected_order_id);
	              	 		...
	              	 		if(!empty($check_exist_product_and_quantity)){
	              	 			...
	              	 			#check if product id is  exist  but quantity is updated then  exist quantity will be update of orderd product.
	              	 			 ...
	              	 			 if($new_order_data['product_id']  == $check_exist_product_and_quantity[0]['product_id'] && $new_order_data['product_quantity'] !=  $check_exist_product_and_quantity[0]['product_quantity']){
		              	 			...
		              	 			$update_order_detail_array = array(
					              	 				'product_quantity'=>$new_order_data['product_quantity'],
					              	 				'updated_at' => time()
					              	 			);
		              	 		...
		              	 		$insert_data = array(
					              	 				...
					              	 				'product_unit_price' => $new_order_data['product_unit_price'],
					              	 				'created_at' => time(),
					              	 				'updated_at' => time()
					              	 			);
              	 			 	 ...
              	 			 	 if($check_exist_product_variant_if_single_select >0 ){
              	 			 	 		//delete =   as for now  will not do delete only will do status 1 
              	 			 	 		 $delete_status =  $this->Common->updateData('order_product_variant_details',array('status'=>1),'product_id = '.$new_order_data['product_id'].' AND order_id = '.$selected_order_id.' AND  variant_id = '.$new_order_data['variant_id'].' ');
              	 			 	 		 	...
              	 			 	 		 	$insert_variant_data = array(
				              	 								'order_id' => $selected_order_id,
					              	 							'product_id' =>$new_order_data['product_id'],
					              	 							'variant_id' =>$new_order_data['variant_id'],
					              	 							'variant_type_id' =>$new_order_data['variant_type_id'],
					              	 							'variant_price' =>$new_order_data['variant_type_unit_price'],
								              	 				'created_at' => time(),
								              	 				'updated_at' => time()
								              	 			);
	...
	public function checkout_after_customize_submit(){
		...
		$update_order_table = [
            		'promo_subtotal_discounted_value' => $items_total_after_promo_code_applied,// if any other wise 0
            		...
            		'dc_amount' => $subtotal_of_delivery,//after promo code applied applied if any
            		'item_quantity' => $item_quantity,
            		'outstanding_amount' => $outstanding_amount_after_customize,
            		'who_will_pay_outstanding_amount' => $who_will_pay_outstanding_amount,//2- restaurant will pay to customer, 3 - customer will pay to restaurant,default 0
            		'updated_by' => $this->role,
            		'checkout_status_by_admin' => 0,
            		'admin_checkout_delivery_charge_if_change' => 0,
	            	'updated_at' => time(),
            	];
		...
		if($update_status > 0){
			...
			if( $who_will_pay_outstanding_amount == 2){// means outstandiing will automatic add in customer wallet from restaurant side
	        		...
	        		$insert_wallet_table = [
	        		  ...
	        		  'order_id' => $selected_order_id,
	        		  'wallet_date' => time(),
	        		  'credited' =>  $outstanding_amount_after_customize,
	        		  'type' =>  2,//1 - Cashback 2 - Money Added 3 debited
	        		  'added_by' => $this->role,//1 - By Admin 2 - By Customer
	        		  'created_at' => time(),
	        		  'updated_at' => time(),
	        		...
	        		if($insert_status >0 ){
	        			...
	        			$paid_status = [
				            		...
				            		'is_paid_outstanding_amount' => 1,//1 for paid ,0  for unpaid
					            	'updated_at' => time(),
			            	];
	...
	public function select_product_variants_for_order($selected_product_id = ""){
...
*/
			...
			if(empty($product_variant_data)){
				...
				// variant available  
				 $pageData['get_checked_variant_data'] = $this->Common->getData('admin_customize_product_variant_cart','variant_type_id','product_id = '.$selected_product_id.'');
 			...
 			if($selected_product_v_id != "" && $selectd_variant_type_id != "" && $order_product_change_status == 1){
 				...
 				//value 1 -  it is "Single select" then selection type will be work as radio button
 					...
 					// after delete if any new unique entry will be insert for one product	
 						$insert_variant_data = array(
					 							'order_id' =>$selected_order_id,
					 							'product_id' =>$selected_product_v_id,
					 							'variant_id' =>$selectd_variant_id,
					 							'variant_type_id' =>$selectd_variant_type_id,
					 							'variant_price' =>$variant_type_price,
				          	 			);
	 		  		...
	 		  		if($mode == 1){
			 			...
			 			if(empty($check_customize_data)){
			 				$insert_variant_data = array(
				              	 							...
				              	 							'variant_type_id' =>$varaint_value['variant_type_id'],
				              	 							'variant_price' =>$varaint_value['variant_price']
           ...
           $customize_product_detail_array = $admin_customize_product_cart;
        		...
        		if(!empty($customize_product_detail_array))
        		{
        			foreach ($customize_product_detail_array as $key => $value) 
        			{
		        		...
		        		$order_product_varirant_query = "SELECT variants.variant_name,variant_types_for_products.single_select,variant_types.variant_type_name,variant_types.variant_type_id,variant_types.variant_id,admin_customize_product_variant_cart.variant_price
		        		FROM admin_customize_product_variant_cart 
		        		INNER JOIN orders ON orders.id = admin_customize_product_variant_cart.order_id 
		        		INNER JOIN `variants` ON `variants`.`variant_id` = `admin_customize_product_variant_cart`.`variant_id` 
		        		INNER JOIN `variant_types_for_products` ON `variant_types_for_products`.`variant_id` = `admin_customize_product_variant_cart`.`variant_id` 
		        		INNER JOIN `variant_types` ON `variant_types`.`variant_type_id` = `admin_customize_product_variant_cart`.`variant_type_id` 
...
/*
	...
	public function ad_banner_list($table = '', $fromdate='all',$todate='all', $banner_status='all',$search_restaurant_id = 'all' ,$search_key='all'){
		...
		if($this->id && $this->role == 1){
            ...
            $data = array(
				'title' => "Ad Banner List",
				'pageName' => "ad-banner-list"
			);
	...
	public function promotion_cashback() {
		...
		if($this->id && $this->role)
        {
			...
			$data = array(
				'title' => "Promotion CashBack",
				'pageName' => "promotion_cashback"
			);
    ...
    public function update_order_cashback_of_promotion()
    {
        if($this->id)
        {
            ...
            $update_status = array(
                'order_cashback' => $order_cashback_type//1 : Flat 2 percent
            );
	...
	public function auth(){
		...
		if(!$this->id){
			 ...
			 if($this->input->post('email')){
	            ...
	            $check_email = $this->Common->getData("users","*","email = '$email' and role = 1 and status NOT IN(3,5)");
	            ...
	            if(!empty($check_email)){
	                if(!$this->bcrypt->check_password($this->input->post('password'), $check_email[0]['password'])){
	                    ...
	                    {
	                        $admin_data = array(
	                                'adminId'  => $check_email[0]['id'],
	                                'email'     => $check_email[0]['email'],
	                                'role' => $check_email[0]['role'],
	                                'fullname' => $check_email[0]['fullname'],
	                                'profile_image' => $check_email[0]['profile_pic'],
	                                'logged_in_restaurant_id' => $logged_in_restaurant_id,
	                                'logged_in_restaurant_name' => $logged_in_restaurant_name,
	       ...
	       // CHECK IF given email is already exist
	        if(!empty($check_email))
	        {
				...
				if($target  == $get_admin_data){
						 ...
						 //upload image -------start
	                        ...
	                        {  
	                            $update_array = [
	                                ...
	                                'email' => $email,
	                                'mobile' => $mobile,
	                                'profile_pic' => $resize_image_path.$fileName,
                 ...
                 $admin_data = array(
	                       'email'     => $get_updated_admin_data[0]['email'],
	                       'fullname' => $get_updated_admin_data[0]['fullname'],
	                       'mobile' => $get_updated_admin_data[0]['mobile'],
	                       'profile_image' => $get_updated_admin_data[0]['profile_pic'],
	               );
                  $this->session->set_userdata($admin_data);
    ...
    public function update_admin_basic_settings()
    {
        if($this->id)
        {
            ...
            $update_value = array(
                ...
                'instagram' => $insta_value,
                'google' => $google_plus,
                'android_version' => $android_version,
                'ios_version' => $ios_version,
                'support_call' => $support_call,
                'support_email' => $support_email,
                'merchant_play_store_url' => $merchant_play_store_value,
                'merchant_app_store_url' => $merchant_app_store_value,
                'customer_play_store_url' => $customer_play_store_value,
                'customer_app_store_url' => $customer_app_store_value,
                'website' => $website_url,
                'basic_delivery_time' => $basic_delivery_time,
                'basic_preparation_time' => $basic_preparation_time,
                'kerela_eats_commission' => $kerela_eats_commission,
    ...
    public function common_multiple_image_upload_function($uploaded_FILES_array,$upload_destiny){
		...
		foreach ($uploaded_FILES_array as $upload_file_name => $upload_file_value) {
		   ...
		   // upload settings
	        ...
	        $config = array(
	            'upload_path' => $path.$upload_file_name,//$upload_file_name is folder name
		...
		// check  in ABOVE threes image that which one is upload or exist image----END---
			...
			$check_exist_user_restaurant = $this->Common->getData('restaurants','id','admin_id = "'.$user_id.'" AND rest_status != 3');
		   	if(!empty($check_exist_user_restaurant)){//check IF ---START----
	    		...
	    		if($user_id != "" && $restaurant_name != ""  && $business_type != "" && (($food_type != "" && $business_type == 1 )|| ($food_type == ""  && $business_type != 1))&&  $description != "" && $postal_code != "" && $pin_address != "" && $unit_number != ""  && $check_accept_type_select_or_not>0 && $open_time != "" && $close_time !=""  && $res_latitude != "" && $res_longtitude !="" && $delivery_handled_by != ""){//Form data is required IF---START---
		    			...
		    			if($logo_upload_valid == 1 &&  $banner_upload_valid == 1 &&  $mobile_banner_upload_valid == 1){
					      ...
					      /*  if($update_logo_image != "" && $update_banner_image != "" && $update_mobile_banner_image != ""){*/
				                ...
				                $merchant_insert = [
				                	...
				                	'food_type' => trim($food_type),//default is 0 ,If restaurant's business type is food then this column will contain value. So value 0 (if not applicable) 1 (Restaurant) 2 (Kitchen)
				                	'res_description' => trim($description),
				                	'rest_pin_address' => trim($pin_address),
				                	'rest_unit_number' => trim($unit_number),
				                	'rest_postal_code' => trim($postal_code),
				                	'is_order_now_accept' => $order_now_value,
				                	'is_self_pickup_accept' => $self_pickup_value,
				                	'is_order_later_accept' => $order_later_value,
				                	'is_dinein_accept' => $dinein_value,
				                	'delivery_handled_by' => $delivery_handled_by,//1 - restaurant 2 - By Kerala Eats
				                	...
				                	'open_time' => $final_open_time,
				                	'close_time' => $final_close_time,
				                	'break_start_time' => $final_break_start_time,
				                	'break_end_time' => $final_break_end_time,
				                	'logo_image' =>  trim($update_logo_image),
				                	'banner_image' =>  trim($update_banner_image),
				                	'mobile_banner_image' =>  trim($update_mobile_banner_image),
				                	'created_at' => time(),
				                ];	
   	...
   	$user_id = $this->db->escape_str(trim($this->input->post('user_id')));
		...
		$check_exist_user_restaurant = $this->Common->getData('restaurants','id','admin_id = "'.$user_id.'" AND id != "'.$restaurant_id.'" AND rest_status != 3');
	   	if(!empty($check_exist_user_restaurant)){//check IF ---START----
		    ...
		    #All Form data is required and should not empty
    		if($user_id != "" && $restaurant_name != "" && $description != "" && $postal_code != "" && $pin_address != "" &&  $unit_number != ""   && $check_accept_type_select_or_not>0 && $res_latitude != "" && $res_longtitude !="" && $delivery_handled_by != ""){//Form data is required IF---START--- // && $final_logo_image != '' && $final_banner_image != '' && $final_mobile_banner_image != ''
  	 ...
  	 public function active_inactive_restaurant(){
  	 	...
  	 	$enable_disable_status = $this->db->escape_str(trim($this->input->post('enable_disable_status')));
  	 	 ...
  	 	 $update_array = [
	                    ...
	                    'rest_status'=>$enable_disable_status, //1 - Enable 2 - Disable 3 - Deleted
  	...
  	// Restaurant Delete -------------------- START----------
  	 public function delete_restaurant(){
  	 	...
  	 	$restaurant_id = $this->db->escape_str(trim($this->input->post('restaurant_id')));
  	 	 ...
  	 	 $update_array = [
	                    'rest_status'=>3,// 1 - Enable, 2 - Disable, 3 - Deleted
	                    'updated_at' => time()
	                ];
        # update data in restaurant table
        $update_status =  $this->Common->updateData('restaurants',$update_array,"id = ".$restaurant_id);
        ...
        if($update_status > 0){
        	...
        	if(!empty($get_merchant_id)){
      		 $update_array = [
                    'status'=>5,// 1 - Enable, 2 - Disable, 3 - Deleted
                    'updated_at' => time()
                ];
  	...
  	//Show Restaurant ---------Update Commission of restaruant -------START-----
  	  public function update_restaurant_commission(){
  	 	...
  	 	if($get_exist_commission_res_data[0]['commission_type'] == $rest_commission_type &&  $get_exist_commission_res_data[0]['commission_value'] == $rest_commission_value){
  	 		...
  	 		$update_array = [
		                    'commission_type'=>	$rest_commission_type , //1- flat/fixed, 2 - percent =  default 0
		                    'commission_value'=>$rest_commission_value, 
		                    'updated_at' => time()
		                ];
	        # update data in restaurant table
	        ...
	        $rest_id = $this->db->escape_str(trim($this->input->post('selected_restaurant_id')));
			    ...
			    if($offline_type == 1) # GOING OFFLINE
			    {
			    	...
			    	$this->Common->deleteData('rest_offline','rest_id = "'.$rest_id.'"');
        				...
        				if($insert_status >0 ){
        					$update_array['is_going_offline'] = 1;
        					$update_status = $this->Common->updateData('restaurants',$update_array,"id = ".$rest_id);
        					if($update_status > 0){
        						echo 1;
        					}else{
	...
	//Show Restaurant ---------Update Delivery Charge per km if hanlde by restaurant-------START-----
  	  public function update_rest_delivery_per_km_charge(){
  	 	...
  	 	if($get_delivery_per_km_charge[0]['per_km_charge'] == $delivery_per_km_charge){
  	 		...
  	 		$update_array = [
		                    'per_km_charge'=> $delivery_per_km_charge , //If delivery handled by restaurant then this will contain per kilo meter charge value
		                     'updated_at' => time()
		                ];
	        # update data in restaurant table
	        $update_status =  $this->Common->updateData('restaurants',$update_array,"id = ".$selected_restaurant_id);
  	...
  	//Show Restaurant ---------Update basic Order  Preparation Time  and delivery Time  of restaruant -------START-----
  	  public function update_order_preparation_and_delivery_time(){
  	 	...
  	 	if($get_exist_preparation_time[0]['preparation_time'] == $rest_order_preparation_time){
  	 		...
  	 		$update_array = [
		                    'preparation_time'=> $rest_order_preparation_time , //Ex 30 so 30 minutes
...
# Common function for pagination start
    ...
    public function restaurant_list($table_data='', $fromdate='all',$todate='all', $resturant_status='all' ,$search_key='all', $res_rating ='all',$business_type = 'all',$food_type = 'all')
    {
        if($this->id)
        {
           ...
           $restaurant_list_data = $this->Common->custom_query($query,'get');
            ...
            $url = base_url('admin/restaurant_list/'.$fromdate.'/'.$todate.'/'.$resturant_status.'/'.$search_key.'/'.$res_rating.'/'.$business_type.'/'.$food_type);
...
public function exportRestaurantCSV($fromdate='all',$todate='all', $resturant_status='all' ,$search_key='all', $res_rating ='all',$business_type = 'all',$food_type = 'all',$restaurant_food_type_is_veg= 'all'){
	   ...
	   fputcsv($file, $header);
	    if(count($restaurantData) > 0){
		   foreach ($restaurantData as $key=>$line){ 
				...
				if($line['food_type'] == 0){ $food_type =  'Food';} 
				if($line['food_type'] == 1){ $food_type =  'Restaurant';} 
				if($line['food_type'] == 2){ $food_type =  'Kitchen';}

				if($line['is_veg'] == 1){ $is_veg =  'Veg';} 
				if($line['is_veg'] == 2){ $is_veg =  'Non Veg';}

				if($line['business_type'] == 1){ $merchant_category =  'Food';} 
				if($line['business_type'] == 2){ $merchant_category =  'Grocery';} 
				if($line['business_type'] == 3){ $merchant_category =  'Alcohol';} 
  	...
  	//Enable /Disable(active/inactive) toggle of Restaurant------ START------
  	 ...
  	 public function active_inactive_promo_code(){
	      ...
	      $enable_disable_status = $this->db->escape_str(trim($this->input->post('enable_disable_status')));
	       ...
	       $update_array = [
	                      'promo_status'=>$enable_disable_status, //1 - Enable, 2 - Disable, 3 - Deleted
   ...
   public function delete_Promo_Code(){
    ...
    $promo_code_name = $this->db->escape_str(trim($this->input->post('promo_code_name')));
     ...
     $update_array = [
                    'promo_status'=>3,// 1 - Enable, 2 - Disable, 3 - Deleted
                    'updated_at' => time()
                ];
  ...
  //Promo Code Delete ------ END------
	...
	public function show_restaurant_product_or_category_according_promo_code_name() {
		...
		if($checked_restaurant_id != ""){
	      ...
	      $query = 'SELECT `restaurant_id`,`applied_on_id` FROM `promotions` WHERE `promotions`.`code_name` = "'.$promotion_code_name .'" '.$query_part.' AND `level_id` = "'.$level_id.'" AND `promotions`.`promo_status` != 3';
	...
	//Add Edit Promotion/Promocode Function  VIEW-----START-------- 
		public function add_edit_promotion_details($type) {
			...
			if($this->id){
				...
				$promo_code_end_date = $this->db->escape_str(trim($this->input->post('promo_code_end_date')));
				...
				$data = [
                    'promo_type' => $promotion_type, //	1 - Flat 2 - Percent
                    'code_name' => $promo_code,
                    'discount_value' => $discount_value,
                    'desciption' => stripslashes($description),
                    'valid_from' => strtotime($promo_code_start_date),
                    'valid_till' => strtotime($promo_code_end_date),
                    'min_value' => $minimum_order_amount,
                    'max_discount' => $max_discount_value,
                    'max_delivery_discount' => $max_delivery_charge,//  if promo applied on deilvery charge and max discount value is given then value will be insert otherwise it will be 0
                    'level_id' => $applied_on,
                    'is_auto_apply' => $promotion_applicaion_mode,
                    'allow_multiple_time_use' => $allow_single_user,
                    'max_allowed_times' => $max_allowed_time,
                    'promotion_mode_status' =>  1,//1- Promo Code, 2- Discount, 3 - Referral
                    'created_at' => time(),
                    'updated_at' => time()
                ];
	...
	//change discount status  (enable/disable)------------------START----------
  	 public function active_inactive_discount(){
	      ...
	      $discount_status_value = $this->db->escape_str(trim($this->input->post('discount_status_value')));
	       ...
	       $update_array = [
	                      'promo_status'=>$discount_status_value, //1 - Enable, 2 - Disable, 3 - Deleted
	                      'updated_at' => time()
	                  ];
	        # update data in promo code_ table
	...
	public function set_edit_discount($type = "" ) {
		...
		if($this->id){
		     ...
		     if($restaurant_id != "" && $discount_name != "" && $discount_value != "" && $description != "" && $max_amount != "" && $discount_start_date != "" && $discount_end_date != ""){
			      ...
			      $data = [
		                    'promo_type' => 2,//	1 - Flat 2 - Percent (Discount is always %)
		                    'code_name' => $discount_name,
		                    'discount_value' => $discount_value,
		                    'desciption' => $description,
		                    'valid_from' => strtotime($discount_start_date),
		                    'valid_till' => strtotime($discount_end_date),
		                    'restaurant_id' => $restaurant_id,
		                    'max_discount' => $max_amount,
		                     'level_id' => 5,//promotion level from promotion_level table 1 - Delivery Charge, 2 -Restaurant, 3- Product, 4- Category, 5 - Global, 6 - Global_Product
		...
		if($this->id && $this->role == 1){
		    ...
		    if($language != "" && $referral_type != "" && $referrer_discount_value != "" && $referrer_max_discount_value != "" && $referrer_discription != "" && $referee_discount_value != "" && $referee_max_discount_value != ""  && $referee_discription != "" && $minimum_order_amount != "" && $type != ""){
		    	 ...
		    	 $data = [
	                    ...
	                    //for referrer-----------
	                    'referrer_discount_value' => $referrer_discount_value,
	                    'referrer_max_discount' => $referrer_max_discount_value,
	                    'referrer_discription' => $referrer_discription,

	                    //for referee-----------
	                    'referee_discount_value' => $referee_discount_value,
	                    'referee_max_discount' => $referee_max_discount_value,
	                    'referee_discription' => $referee_discription,
	...
	//change referral status  (enable/disable)------------------START----------
     public function active_inactive_referral(){
        ...
        $referral_status_value = $this->db->escape_str(trim($this->input->post('referral_status_value')));
         ...
         $update_array = [
                        'promo_status'=>$referral_status_value, //1 - Enable, 2 - Disable, 3 - Deleted
                        'updated_at' => time()
                    ];
          # update data in promo code_ table
  ...
  //change referral status  (enable/disable)------------------END----------
	    			...
	    			if($ad_banner_name != ""   && $ad_banner_description != "" && $_FILES['web_banner_image']['name'] != '' && $_FILES['web_mobile_banner_image']['name'] != '' && $_FILES['mobile_banner_image']['name'] != ''){//Form data is required IF---START---
					   ...
					   //Check LOGO and BANNER  and mobile Banner upload status is true, then data should be insert
						if ((count(array_unique($ad_banner_upload_status)) === 1 && end($ad_banner_upload_status) === 'true') && empty($ad_banner_upload_error)){// array all upload status should be same  ---IF START ---
							...
							# Now make entry to RESTAURANT table
							$ad_banner_insert = [
								'restaurant_id' => $selected_restaurant_id,// if select then value be post other wise it will blank
								'ad_image' => 'assets/images/ad_banners/web_banner_image/'.$ad_banner_file_name['FileNameOf_web_banner_image'],
								'ad_image_web' => 'assets/images/ad_banners/web_mobile_banner_image/'.$ad_banner_file_name['FileNameOf_web_mobile_banner_image'],
								'ad_image_mobile' => 'assets/images/ad_banners/mobile_banner_image/'.$ad_banner_file_name['FileNameOf_mobile_banner_image'],
								'ad_name' => $ad_banner_name,
								'ad_description' => $ad_banner_description,
								'external_ink' => $external_link,
								'created_at' => time(),
								'updated_at' => time(),
							];	
							...
							if($web_banner_upload_valid == 1 &&  $web_mobile_banner_upload_valid == 1 &&  $mobile_banner_upload_valid == 1){
							   ...
							   //check WEB BANNER and  mobile app banner upload only and web  mobile banner are exist file, its all are exist file--------End-------
							     ...
							     if($update_web_banner_image != "" && $update_web_mobile_banner_image != "" && $update_mobile_banner_image != ""){
										...
										$ad_banner_update = [
											'restaurant_id' => $selected_restaurant_id,// if select then value be post other wise it will blank
											'ad_image' => trim($update_web_banner_image),
											'ad_image_web' => trim($update_web_mobile_banner_image),
											'ad_image_mobile' => trim($update_mobile_banner_image),
											'ad_name' => $ad_banner_name,
											'ad_description' => $ad_banner_description,
											'external_ink' => $external_link,
											'updated_at' => time(),
   ...
   public function delete_ad_banner(){
    ...
    $update_array = [
                    'status'=>3,// 1 - Enable, 2 - Disable, 3 - Deleted
                    'updated_at' => time()
                ];
      # update data in promotion table
   ...
   // Ad  banner Enable / Disable -----------------------START--------------------
    ...
    $update_array = [
                    'status'=> $status_value, // 1 - Enable, 2 - Disable, 3 - Deleted
                    'updated_at' => time()
                ];
      # update data in promotion table
      $update_status =  $this->Common->updateData('ad_banners',$update_array,"id = ".$ad_banner_id." ");
		                     ...
		                     if(!empty($_FILES['merchant_profile_image']['name'])){
		                        ...
		                        {       
		                            $data = [
		                                'fullname' => $fullname,
		                                'email' => $email,
		                                 'password' => $this->bcrypt->hash_password($GeneratedPassword),
		                                'mobile' => $mobile,
		                                'role' => 2,
		                                'profile_pic' => $resize_image_path.$fileName,
		                                'created_at' => time(),
		                  ...
		                  // if exist image only data change
					   		...
					   		if($merchant_profile_image != ''){
					   			$update_array = [
	                                ...
	                                'email' => $email,
	                                'mobile' => $mobile,
	                                 'profile_pic' => $merchant_profile_image,
	                                'updated_at' => time()
                           		 ];
					   		}else{
					   			$update_array = [
  ...
  //Change user status-------- START----------
   ...
   public function edit_user_status($selected_role = ""){
    ...
    $update_array = [
                    'status'=> $user_status,// 1 - Enable, 2 - Disable, 3 - Deleted
                    'updated_at' => time()
                ];
      # update data in user table
		   	...
		   	if(!empty($check)){//check IF ---START----
		   		...
		   		echo 3;
					   		...
					   		$update_array = [
                                ...
                                'email' => $email,
                                'mobile' => $mobile,
                                'user_pin_address' => $customer_pin_address,
                                'user_unit_number' => $unit_number,
                                'user_postal_code' => $postal_code,
                                'latitude' => $customer_latitude,
                                'longitude' => $customer_longtitude,
                                'updated_at' => time()
	...
	public function add_money_in_customer_wallet() {
		...
		if($customer_id != "" && $customer_credit_amount !="'"){
			$insert_wallet_table = [
    		  'user_id' => $customer_id,
    		  'wallet_date' => time(),
    		  'credited' =>  $customer_credit_amount,
    		  'type' =>  2,//1 - Cashback 2 - Money Added 3 debited
    		  'added_by' => $this->role,//1 - By Admin 2 - By Customer
    		  'created_at' => time(),
    		  'updated_at' => time(),
    		...
    		if($insert_status  > 0){
              ...
              $notification_data_fields = array(
                    'message' => 'S$'.$customer_credit_amount.' added to your wallet',
                ...
                # Now insert notification to Database FOR MERCHANT ID
                $insertData = [
                	'title' => 'S$'.$customer_credit_amount.' added to your wallet',
                	'to_user_id' => $customer_id,
                	'type' => 2, # 2 - Payment related ( basically to restaurant) 
                	'is_read' => 1,
                	'created_at' => time(),
                	'updated_at' => time(),
                ];
	...
	public function deduct_money_from_customer_wallet() {
		...
		if($customer_id != "" && $customer_debit_amount !="'"){
			$insert_wallet_table = [
    		  'user_id' => $customer_id,
    		  'wallet_date' => time(),
    		  'debited' =>  $customer_debit_amount,
    		  'type' =>  3,//1 - Cashback 2 - Money Added 3 debited
    		  'added_by' => $this->role,//1 - By Admin 2 - By Customer
    		  'created_at' => time(),
    		  'updated_at' => time(),
    		];
    		...
    		if($insert_status  > 0){
               ...
               //getting user device token  for send notification
                ...
                $notification_data_fields = array(
                    'message' => 'S$'.$customer_debit_amount.' Deduct from your wallet',
                    'title' => NOTIFICATION_MONEY_DEDUCTED,
                ...
                # Now insert notification to Database FOR MERCHANT ID
                $insertData = [
                	'title' => 'S$'.$customer_debit_amount.' Deduct from your wallet',
                	'to_user_id' => $customer_id,
                	'type' => 2, # 2 - Payment related ( basically to restaurant) 
                	'is_read' => 1,
                	'created_at' => time(),
 ...
 //Show particular user detail ----------START--------------
	...
	public function user_details($user_role = '', $user_id = '') {
		...
		if($this->id && $this->role == 1){
			...
			$data = array(
		        'title' => "User Details",
				'pageName' => "show-user-detail"
			);
		
           $pageData['user_detail'] = $this->Common->getData('users','id,role,	number_id, fullname, email, mobile, profile_pic, device_type,status ,user_pin_address,user_unit_number,user_street_address,user_postal_code,created_at','id = '.$user_id.'  AND role = '.$user_role.'  AND status != 5');
	...
	public function products($table = '', $fromdate='all',$todate='all', $product_status='all', $product_food_type='all' ,$search_key='all',$restaurant_id= 0,$category_id = 0) {
		...
		if($this->id){
			...
			if($table!= "" || $fromdate!="all" || $todate!="all" || $product_status!="all" || $search_key != "all" || $restaurant_id != 0 || $category_id != 0)
					...
					$pageData['product_detail'] = $this->Common->custom_query($query,'get');
					...
					$pageData['category_detail'] = $this->Common->custom_query($query_for_category,'get');
	        ...
	        if($offline_type == 1) # DISABLE
			    {
			    	...
			    	if($offline_tag == ''){
			        	...
			        	//---------------------------------
							...
							$update_array = [
							              'product_status'=>2, //1 - Enable, 2 - Disable, 3 - Deleted
							              'updated_at' => time()
							          ];
							# update data in product  table
							$update_status =  $this->Common->updateData('products',$update_array,"restaurant_id = ".$rest_id." AND  id = ".$product_id);
			    ...
			    }else if($offline_type == 2) # TO ENABLE
			    {
			    	...
			    	//---------------------------------
							...
							$update_array = [
							              'product_status'=>1, //1 - Enable, 2 - Disable, 3 - Deleted
							              'updated_at' => time()
							          ];
							# update data in product  table
							$update_status =  $this->Common->updateData('products',$update_array,"restaurant_id = ".$rest_id." AND  id = ".$product_id);
	     ...
	     if($offline_type == 1) # DISABLE
		            ...
		            }else
		            {
    					...
    					*/
        				...
        				foreach ($products as $product) 
        				{
    						...
    						$this->Common->updateData('products',$update_products_arrray,'category_id = "'.$category_id.'" AND restaurant_id = '.$rest_id.' AND id = "'.$product['id'].'"');
    						 
        				}

        		        //category will also disable
	        			$update_category_array = [
		                      'category_status'=> 2, //1 - Enable, 2 - Disable, 3 - Deleted
		                      'updated_at' => time()
		                  ];

        				$update_status = $this->Common->updateData('categories',$update_category_array,"id = ".$category_id);
			    ...
			    }else if($offline_type == 2) # TO ENABLE
			    {
			    	...
			    	$this->Common->deleteData('products_offline','rest_id = "'.$rest_id.'" AND category_id = "'.$category_id.'"');
			    		...
			    		//category also Enable
	        			$update_category_array = [
		                      'category_status'=> 1, //1 - Enable, 2 - Disable, 3 - Deleted
		 
		                      'updated_at' => time()
		                  ];

        				$category_update_status = $this->Common->updateData('categories',$update_category_array,"id = ".$category_id);
					 
        				if($category_update_status > 0){

        					$check_products = $this->Common->getData('products','id','category_id = "'.$category_id.'" AND restaurant_id = "'.$rest_id.'"');
    						if(count($check_products) > 0) # That means this product already exists in that category it means it will also disable
    						{
						        ...
						        # update data in product  table
						         $final_update_status =  $this->Common->updateData('products',$update_product_array,"restaurant_id = ".$rest_id." AND category_id = ".$category_id);
		    				}else{
		    					$final_update_status = 1;// when product will add then that inserted time product will insert with status 1 means enable 
		    				}
        				}else{
        					$final_update_status = 1;
        				}

        				if($final_update_status > 1 || $final_update_status == 1){
        				   echo 1;//update_success
        				}else{
        				   echo 0;
        				}
			    }
  	...
  	// Product Delete -------------------- START----------
	   public function delete_product(){
	    ...
	    $product_id = $this->db->escape_str(trim($this->input->post('product_id')));
	     ...
	     $update_array = [
	                    'product_status'=>3,// 1 - Enable, 2 - Disable, 3 - Deleted
	                    'updated_at' => time()
	                ];
	      # update data in promotion table
  ...
  // category Delete -------------------- START----------
     public function delete_category(){
      ...
      $category_id = $this->db->escape_str(trim($this->input->post('category_id')));
       ...
       $update_array = [
                      'category_status'=>3,// 1 - Enable, 2 - Disable, 3 - Deleted
                      'updated_at' => time()
                  ];
			                   ...
			                   if($file_name_array[0] == 'product'){// in product tab wrongly uplaod combine or category  file then it will be not run
				                  ...
				                  $product_if_exist_data = $this->Common->custom_query($get_product_if_exist_by_name_query,'get');
				                   ...
				                   $data = [
			                                ...
			                                'is_veg' => $food_type,//	1 - Veg 2 - Non veg
			                                'price' => $product_price,
			                                'offer_price' => $product_offer_price,
			                                'min_qty' => $minimum_quantity,//MINIMUM QUANTITY TO ORDER. Default is 1
			                                'short_desc' => $product_short_discription,
			                                'long_desc' => $product_long_discription,
		                    ...
		                    if(!empty($_FILES['category_image']['name']) ){
		                        ...
		                        {       
		                              ...
		                              $data = [
			                                ...
			                                'category_name' => $category_name,
			                                'description' => $category_description,
			                                'category_image' => $resize_image_path.$fileName,
			                                'updated_at' => time()
			                            ];
		                     		...
		                     		//if image uploaded
				                        ...
				                        {       
				                            ...
				                            $data = [
				                                ...
				                                'short_desc' => $product_short_discription,
				                                'long_desc' => $product_long_discription,
				                                'product_image' => $resize_image_path.$fileName,
				                                'updated_at' => time()
  ...
  //Add and edit product deatils---------------------------END-----------
   ...
   { 
    	if($this->id && $this->role == 1) # Only admin can do this
		{
			...
			if($page_name != "" && $page_key != ""  && $page_primary_id != ""){
				$update_array = [
   ...
   //Admin/ Merchant Update Password ----------START---------
	...
	public function notification(){
		/** Google URL with which notifications will be pushed */
			 ...
			 */
			$subscription_key  = "key=AAAAV2StWI8:APA91bEMg0DDBoEO0kVMjyqKLnB9WGURRlt0TtoeaImPJcZfFS__XsTj-FeHGPrzSEiHfF8_AYXWAx_UxPHIt5_AU98r6ojH93XjIS1OgEjUBLu-EJcZANG7WpXdiAHYBPEv4Y-axLbH"; // firebase web key
			...
			$postRequest = [
			    ...
			    "to" =>  "dF5E7F1ZrPUxadbv9FAPMY:APA91bEztcO59SaMVaQdBOuRQ7lexwDDa0Spjymn6TgMXNPQpCe4cwCRrlYYPITn6_IYM0KIdgfrR9JPjorbDgOrffwJPqs_NLN1hPPZW4qhJXcqkyrSZAXU51Xz3IHEHoTrIdaNaqFC" //device token 
			];
