...
class Admin extends CI_Controller {
    ...
    public function orders($table = '', $customer_id = "", $fromdate = 'all', $todate = 'all', $delivery_handle_by = 'all', $payment_mode = 'all', $paid_status = 'all', $is_paid_to_restaurant = 'all', $order_status = 'all', $search_restaurant_id = 'all', $search_customer_id = 'all', $order_accept_type = 'all', $business_category_id = 'all', $is_cutlery_needed = 'all', $is_promocode_auto_applied = 'all', $is_promocode_auto_applied_on_delivery = 'all', $search_key = 'all', $schedule_dt = 'all', $schedule_dt_to = 'all') {
        if ($this->id != '' && $this->role == 1 || $this->role == 2) {
            ...
            if ($fromdate != 'all' || $todate != 'all' || $delivery_handle_by != 'all' || $payment_mode != 'all' || $paid_status != 'all' || $is_paid_to_restaurant != 'all' || $order_status != 'all' || $search_restaurant_id != 'all' || $search_customer_id != 'all' || $order_accept_type != 'all' || $business_category_id != 'all' || $is_cutlery_needed != 'all' || $is_promocode_auto_applied != 'all' || $is_promocode_auto_applied_on_delivery != 'all' || $search_key != 'all' || $schedule_dt != 'all' || $schedule_dt_to != 'all') {
                ...
                $order_now_sc_start = $sc_start - 28800; // Subtracting 8 hours because created_at is as per local time
                $order_now_sc_end = $sc_end - 28800; // Subtracting 8 hours because created_at is as per local time
                if ($schedule_dt != "all" && $schedule_dt_to == "all") {
                    $query_part.= " AND IF (`orders`.`order_type` = '1' , `orders`.`created_at`  >= '" . $order_now_sc_start . "' , `orders`.`pickup_time_from` >= '" . $sc_start . "')";
                }
                ...
                if ($schedule_dt != "all" && $schedule_dt_to != "all") {
                    ...
                    $query_part.= " AND IF (`orders`.`order_type` = '1' , `orders`.`created_at` BETWEEN '" . $order_now_sc_start . "' AND '" . $order_now_sc_end . "' , `orders`.`pickup_time_from` BETWEEN '" . $sc_start . "' AND '" . $sc_end . "')";
            ...
            if ($update_status > 0) {
                ...
                if ($order_status == 1) # Only when admin ACCEPTS order 1 - lalamove place order 2 - send cashback to user
                {
                    ...
                    if ($order_type != 2 && $some_checks[0]['delivery_handled_by'] == 2) # That is when it is not a Self pickup order and delivery need to be handeled by Kerala eats
                    {
                        ...
                        $rest_delivery_time = $some_checks[0]['delivery_time'];
                        # If restaurant has not set any delivery time then we need to take value from settings table
                        if ($rest_delivery_time == 0 || $rest_delivery_time == '')
                        {
                            $basic_delv_time = $this->Common->getData('settings', 'value', 'name = "basic_delivery_time"');
                        ...
                        if ($order_type == 1)
                        { 
                            $is_pickup_time_changed = $order_data[0]['is_pickup_time_changed'];
                        ...
                        # Now we have $final_pickup_date_from and $final_pickup_range in date time format but we need only timestamp
                        $final_pickup_range_exp = explode(":",$final_pickup_range);
                        $final_pickup_range_hrs = $final_pickup_range_exp[0];
                        $final_pickup_range_min = $final_pickup_range_exp[1];

                        $send_to_lalamove = strtotime('+' . $final_pickup_range_hrs . 'hours ' .'+'.$final_pickup_range_min.'minutes', strtotime($final_pickup_date_from));
                        ...
                        {
                            $start = new DateTime(date('r', $send_to_lalamove));
                            $send_to_lalamove_final = $start->format('Y-m-d\TH:i:s\Z');
                            $lalamove_support_number = LALAMOVE_SUPPORT_NUMBER;
                            $body = array(
                            "scheduleAt" => $send_to_lalamove_final, // ISOString with the format YYYY-MM-ddTHH:mm:ss.000Z at UTC time
                        ...
                        } else # 3 : Order for later
                        {
                            ...
                            $body = array("scheduleAt" => $send_to_lalamove_final, // ISOString with the format YYYY-MM-ddTHH:mm:ss.000Z at UTC time ## We will make order 15 minutes before for lalamove
                            "serviceType" => "MOTORCYCLE", // string to pick the available service type
                            "specialRequests" => array(), // array of strings available for the service type
                            "requesterContact" => array("phone" => $req_phone, // Phone number format must follow the format of your country
                            "name" => $lalamove_rest_details[0]['rest_name'],), "stops" => array(array("location" => array("lat" => $lalamove_rest_details[0]['latitude'], "lng" => $lalamove_rest_details[0]['longitude']), "addresses" => array("en_SG" => array("displayString" => $lalamove_rest_details[0]['rest_pin_address'] . ", " . $lalamove_rest_details[0]['rest_unit_number'] . ", " . $lalamove_rest_details[0]['rest_postal_code'], "country" => "SG"
                ...
                // die;
                // if ($order_status == 4 || $order_status == 2) # CANCELLED OR REJECTED
                if ($order_status == 4) # CANCELLED ONLY :when we Reject the order, don't need to credit the amount back to the customer's wallet. Only when we cancel the orders. Please help to change back and apologies for the trouble. :
                {
    ...
    public function lalamove_order_deails($lalamove_order_id) {
        ...
        $key = 'da10edb6ed693df6c7baccba92fd2gc308776'; #SANDBOX
        $secret = 'b3aVjAP/dvbi4vxy0PfeagdgBhypakpPMBthrlqDPgwZJJ6tQKlHoyGGm+2EFCf9AK7sQfR'; #SANDBOX
        $time = time() * 1000;
        $method = 'GET';
    ...
    public function lalamove_quotation_generate($body) {
        ...
        $key = 'da10edb6ed6936c7baccdsdsgbcvcva922c308776'; #SANDBOX
        $secret = 'b3aVjAP/dvbi4vxy0PdsgsgsggfeaBpakpPMBtlqDPgwZJJ6tQKlHoyGGm+2EFCf9AK7sQfR'; #SANDBOX
        $time = time() * 1000;
        $method = 'POST';
        $path = '/v2/quotations';
        $region = 'SG';
    ...
    public function lalamove_quotation_place_order($body) {
        ...
        $key = 'da10edb6ed6936c7baccbdsfdsdsfa922c308776'; #SANDBOX
        $secret = 'b3aVjAP/dvbi4vxy0PfedgfdgdfgaBpakpPMBtlhhhqDPgwZJJ6tQKlHoyGGm+2EFCf9AK7sQfR'; #SANDBOX
        $time = time() * 1000;
        $method = 'POST';
        $path = '/v2/quotations';
        $region = 'SG';
    ...
    public function update_order_delivery_address_with_delivery_charge() {
        ...
        $pin_address = $this->db->escape_str(trim($this->input->post('delivery_address')));
        $street_address = $this->db->escape_str(trim($this->input->post('delivery_street_address')));
        ...
        #check address has  changed like postal code, unit number , block number etc.
        #if main delivery address  is changed then we will get delivery amount from lalamove  and show checkout popup. other wise we will  update only other things(postal code, unit number , block numbe) and dont show checkout popup.
        #getting exist address data from order table
        $exist_delivery_address = $this->Common->getData('orders', 'delivery_address,   delivery_street_address,delivery_postal_code,delivery_unit_number', 'id = "' . $selected_order_id . '"');
        $is_anything_is_change_in_address = 0;
        $is_need_to_get_new_delivery_charge = 0;
        if (!empty($exist_delivery_address)) {
            if (trim($exist_delivery_address[0]['delivery_address']) != $pin_address) {
                $is_need_to_get_new_delivery_charge = 1;
            }
            if (trim($exist_delivery_address[0]['delivery_street_address']) != $street_address) {
                $is_anything_is_change_in_address = 1;
            } else if (trim($exist_delivery_address[0]['delivery_postal_code']) != $delivery_postal_code) {
                $is_anything_is_change_in_address = 1;
            } else if (trim($exist_delivery_address[0]['delivery_unit_number']) != $delivery_unit_number) {
                $is_anything_is_change_in_address = 1;
            }
        }
        ...
        if ($order_type != 2 && $delivery_handled_by == 2) { //delivery_handled_by by kerala eatss
            ...
            $update_array = ['delivery_street_address' => $street_address, 'delivery_postal_code' => $delivery_postal_code, 'delivery_unit_number' => $delivery_unit_number, 'updated_at' => time() ];
            //update lalamove address detail in order table if address order address change by admin
            if ($is_need_to_get_new_delivery_charge == 1) {
                $update_array['delivery_address'] = $pin_address;
                $update_array['actual_dc_amount'] = $lalamove_order_amount;
                $update_array['lalamove_order_failed_reason'] = $lalamove_order_response['failed_reason'];
                $update_array['admin_checkout_delivery_charge_if_change'] = $lalamove_order_amount;
                $update_array['delivery_latitude'] = $latitude;
                $update_array['delivery_longitude'] = $longitude;
                // $update_array['checkout_status_by_admin'] = 1; // for checkout modal show for update delivery amount with item calculation
                $update_array['checkout_status_by_admin'] = 0; // #CHSDC
                
            }
            if ($is_anything_is_change_in_address == 1 || $is_need_to_get_new_delivery_charge == 1) {
                $update_array['is_delivery_address_changed'] = 1;
            }
            # update data in orders table
            ...
            if ($is_need_to_get_new_delivery_charge == 1) {
                $update_array['delivery_address'] = $pin_address;
                $update_array['actual_dc_amount'] = $rest_delivery_charge;
                $update_array['admin_checkout_delivery_charge_if_change'] = $rest_delivery_charge;
                $update_array['delivery_latitude'] = $latitude;
                $update_array['delivery_longitude'] = $longitude;
                // $update_array['checkout_status_by_admin'] = 1; // for checkout modal show for update delivery amount with item calculation
                $update_array['checkout_status_by_admin'] = 0; // #CHSDC
    ...
    public function update_order_pick_up_time() {
        ...
        // } else if ($order_type == 3) # Order For Later
            # TAG_EDITPICKUPTIME
        } else if ($order_type == 3 || $order_type == 1) # Order For Later OR order now
        {
        ...
        $update_array = ['pickup_time_from' => $final_pick_up_from, 'pickup_time_to' => $final_pick_up_to, 'updated_at' => time() , 'is_pickup_time_changed' => 1];
        # update data in orders table
        $update_status = $this->Common->updateData('orders', $update_array, "id = " . $selected_order_id);
        // echo $this->db->last_query();
        if ($update_status > 0) {
    ...
    #calucation new updated  item total
    public function calculate_new_updated_item_total($order_product_details) {
        $items_total = 0;
    ...
    public function check_promotion_all_condition($item_total , $promo_data , $user_id,$pickup_time = 'NA')
    {
        ...
        if($response['item_total'] >= $promo_data['min_value']) # This is the only mendatory check
        {
            ...
            if($is_applicable)
            {
                ...
                }else # One user can use it only once
                {
                    ...
                    $usr_used_pc = $this->Common->getData('used_promotions','id',array('promotion_id' => $promo_data['id'] , 'user_id' => $user_id));
    ...
    public function get_delivery_charge_promotion($restaurant_id , $item_total , $user_id = '',$pickup_time = 'NA')
    {
            ...
            foreach ($delivery_promotion_all as $key => $delivery_promotion) 
            {
                ...
                if($item_total >= $delivery_promotion['min_value']) # This is the only mendatory check
                {
                    ...
                    if($is_applicable)
                    {
                        ...
                        {
                            # That is one user can use this only once. So check whether this promo is used by this customer earlier?
                            $usr_used_pc = $this->Common->getData('used_promotions','id',array('promotion_id' =>$delivery_promotion['id'] , 'user_id' => $user_id));
                            if(count($usr_used_pc) > 0)
                            {
                                $is_applicable = false;
                            }else
    ...
    #Checking applicable promotion after new customized item total
    public function checking_applicable_promo_after_customized_item($selected_order_id,$new_item_total,$product_ids){
        #getting rest id 
        $get_order_data = $this->Common->getData('orders', 'restaurant_id,user_id,order_type,pickup_time_from,actual_dc_amount', 'id = "' . $selected_order_id . '"');
        $restaurant_id = $get_order_data[0]['restaurant_id'];
        $user_id = $get_order_data[0]['user_id'];
        $order_type = $get_order_data[0]['order_type'];
        $pickup_time = $get_order_data[0]['pickup_time_from'];
        $actual_dc_amount = $get_order_data[0]['actual_dc_amount'];

        #getting category ids
        $category_ids = array();
        foreach ($product_ids as $key => $product_id) {
            $pro_data = $this->Common->getData('products', 'category_id', 'id = "' . $product_id . '"');
            $category_ids[] = $pro_data[0]['category_id']; 
        ...
        */
        ...
        if(count($global_level_promo) > 0)
        {
            ...
            {
                $promo_response[] = $this->check_promotion_all_condition($new_item_total , $global_level , $user_id,$pickup_time);
            }
            // echo "<pre>promo_response";
            // print_r($promo_response);
            $current_item_total = $new_item_total;
            if(count($promo_response) > 0  ) # Multiple promo available as per multiple categories
            {
                # That mens we have promotion on two categories so we need to find which one has high value of discount
                $check_high_promo = array();
        ...
        if($check_all_restro_promo == 1)
        {
            ...
            if(count($all_restro_promo) > 0)
            {
                ...
                {
                    $promo_response[] = $this->check_promotion_all_condition($new_item_total , $all_restro , $user_id,$pickup_time);
                }
                // echo "<pre>POPPPPO";
                // print_r($promo_response);
                $current_item_total = $new_item_total;
        ...
        if($check_restaurant_promo == 1)
        {
            ...
            if(count($rest_level_promo) > 0)
            {
                ...
                {
                    $promo_response[] = $this->check_promotion_all_condition($new_item_total , $rest_level , $user_id,$pickup_time);
                }
                $current_item_total = $new_item_total;
                if(count($promo_response) > 0 ) # Multiple promo available as per multiple categories
                {
        ...
        if($check_category_promo == 1)
        {
            ...
            foreach($category_ids as $key => $value)
            {
                ...
                if(count($category_level_promo) > 0)
                {
                    ...
                    {
                        $promo_response[] = $this->check_promotion_all_condition($new_item_total , $cat_promo , $user_id,$pickup_time);
                    }
                    $current_item_total = $new_item_total;
                    if(count($promo_response) > 0 ) # Multiple promo available as per multiple categories
                    {
        ...
        echo $check_restaurant_promo ;
        echo $check_category_promo;
        echo $check_product_promo;*/
        ...
        if($check_product_promo == 1)
        {
            ...
            foreach($product_ids as $key => $value)
            {
                ...
                if(count($product_level_promo) > 0)
                {
                    ...
                    {
                        $promo_response[] = $this->check_promotion_all_condition($new_item_total , $prod_promo , $user_id,$pickup_time);
                    }
                    ...
                    if(count($promo_response) > 0)
                    {
                        ...
                        foreach ($promo_response as $keyv => $catprd_value)
                        {
                            if(!empty($catprd_value))
                            {
                                ...
                                {
                                    // $disc_val = $new_item_total - $catprd_value['discount_value'];
                                ...
                                {
                                    $disc_val = ($new_item_total * $catprd_value['discount_value']) / 100;
                                    // $disc_val = $new_item_total - $disc_val;
                                    $disc_val = number_format($disc_val,2, '.', '');
                                    $check_high_promo_prod['high_promo'][$keyv] = array('promo_id' => $catprd_value['id'] , 'disc_val' => $disc_val);
                                }
                            }
        ...
        if($order_type == 2) # Self pickup : So check any promo for selfpickup user available? 
        {
            ...
            if(count($super_admin_self_pickup_promo) > 0)
            {
                ...
                {
                    $promo_response[] = $this->check_promotion_all_condition($new_item_total , $self_pickup , $user_id,$pickup_time);
                }
                $current_item_total = $new_item_total;
            ...
            if($check_merchant_self_pickup_promo == 1)
            {
                ...
                if(count($merchant_self_pickup_promo) > 0)
                {
                    ...
                    {
                        $promo_response[] = $this->check_promotion_all_condition($new_item_total , $mer_self_pickup , $user_id,$pickup_time);
                    }
        ...
        }else
        {
            ...
            if(count($response['promocode_details']) > 0)
            {
                ...
                $actual_item_total = $new_item_total;
                ...
                // $new_item_total = $actual_item_total;
                // $new_item_total = number_format($new_item_total,2, '.', '');
            } # IN ELSE CASE WE ARE ALREADY HAVING $new_item_total made above so if no promo applied then the above generated values will be sent.
            else
            {
                $actual_item_total = $new_item_total;
            }

            //echo 'Sending this value dc'.$actual_item_total;

            # get_delivery_charge_promotion will return object
            // $delivery_promotion = $this->get_delivery_charge_promotion($restaurant_id , $new_item_total , $user_id,$pickup_time);
            $delivery_promotion = $this->get_delivery_charge_promotion($restaurant_id , $actual_item_total , $user_id,$pickup_time);
       ...
       // print_r($response['promocode_details']);
        if(!empty($response['promocode_details'])){
            $promo_id_of_subtotal['promo_id_of_subtotal'] = $response['promocode_details'][0]['id'];//applicable promo_id
            $promo_id_of_subtotal['promo_subtotal_is_applied'] = 1;

            #getting discount value 
            if($response['promocode_details'][0]['promo_type'] == 2){ #percent
                $promo_id_of_subtotal['promo_subtotal_discounted_value'] = $new_item_total * $response['promocode_details'][0]['discount_value'] / 100;
            }else{ #flat
                $promo_id_of_subtotal['promo_subtotal_discounted_value'] = $response['promocode_details'][0]['discount_value'];
            }
        }else{
            $promo_id_of_subtotal['promo_id_of_subtotal'] = 0;
            $promo_id_of_subtotal['promo_subtotal_is_applied'] = 2;
            $promo_id_of_subtotal['promo_subtotal_discounted_value'] = 0;
        }


        /*if(!empty($response['delivery_charge_promotion'])){
            $dc_result = json_decode($response['delivery_charge_promotion'], true);
        }else{
            $dc_result  = 0;
        }*/
        $dc_result =  (array)$response['delivery_charge_promotion'];
        
        if(count($dc_result)>0){
            $promo_of_dc['promo_id_of_dc'] = $response['delivery_charge_promotion']['id'];//applicable promo_id
            $promo_of_dc['promo_dc_is_applied'] = 1;

            #getting discount value 
            if($response['delivery_charge_promotion']['promo_type'] == 2){ #percent
                $promo_of_dc['promo_dc_discounted_value'] = $actual_dc_amount * $response['delivery_charge_promotion']['discount_value'] / 100;
            }else{ #flat
                $promo_of_dc['promo_dc_discounted_value'] = $response['delivery_charge_promotion']['discount_value'];
            }
        }else{
            $promo_of_dc['promo_id_of_dc'] = 0;
            $promo_of_dc['promo_dc_is_applied'] = 2;
            $promo_of_dc['promo_dc_discounted_value'] = 0;
        }
        return  array($promo_id_of_subtotal,$promo_of_dc);
    }
    ...
    public function confirm_and_update_place_order() {
        ...
        if ($remove_duplicate_final_order_items_array) {
            foreach ($remove_duplicate_final_order_items_array as $new_order_data) {
                ...
                #for only promo check only
                $product_ids[] = $new_order_data['product_id'];
            ...
            if (!empty($final_insert_update_status) && in_array(0, $final_insert_update_status) == false) {
                if (in_array(0, $final_insert_update_status) == false) {
                    ...
                    #getting new updated iteam total
                    $order_product_details_array = $this->comman_show_orderd_products_function($selected_order_id, '', '1'); //order id,selected_product_id (Dont need to give on first time view), load mode check

                    $new_item_total = $this->calculate_new_updated_item_total($order_product_details_array);

                    #checking promotion after new updated item total
                    $new_promo_id = $this->checking_applicable_promo_after_customized_item($selected_order_id,$new_item_total,$product_ids);

                    //print_r($new_promo_id);
                    $update_status = $this->Common->updateData('orders', array('checkout_status_by_admin' => 1,'promo_subtotal_is_applied' => $new_promo_id[0]['promo_subtotal_is_applied'],'promo_subtotal_code_id' => $new_promo_id[0]['promo_id_of_subtotal'],'promo_subtotal_discounted_value' => $new_promo_id[0]['promo_subtotal_discounted_value'],'promo_dc_code_id' => $new_promo_id[1]['promo_id_of_dc'],'promo_dc_is_applied' => $new_promo_id[1]['promo_dc_is_applied'],'promo_dc_discounted_value' => $new_promo_id[1]['promo_dc_discounted_value']), 'id = ' . $selected_order_id . '');
    ...
    public function checkout_after_customize_submit() {
        ...
        if ($update_status > 0) {
            # SEND NOTIFICATION TO CUSTOMER AND MERCHANT OF THIS RESTAURANT
            # GET customer id and rest id and then get device tokens
            $not_data = $this->Common->getData('orders', 'user_id, order_number, restaurant_id', 'id = "' . $selected_order_id . '"');
            $user_id = $not_data[0]['user_id'];
            $restaurant_id = $not_data[0]['restaurant_id'];
            $order_number_id = $not_data[0]['order_number'];
            $merchant_id = $this->Common->getData('restaurants', 'admin_id', 'id = "' . $restaurant_id . '"');
            $merchant_id = $merchant_id[0]['admin_id'];
            $token_customer = $this->Common->getData('users', 'device_token', 'id = "' . $user_id . '"');
            $token_merchant = $this->Common->getData('users', 'device_token', 'id = "' . $merchant_id . '"');
            // echo $this->db->last_query();
            $notification_data_fields = array('message' => 'Order ' . $order_number_id . ' is customized', 'title' => NOTIFICATION_TITLE_ORDER_CUSTOMIZED,
            # 0 => pending , 1 => processed , 2 => Packing, 3 => shipped 4 => Completed 5 => Unfulfilled  6 => Delivered
            'order_id' => $selected_order_id, 'notification_type' => 'ORDER_STATUS_UPDATED');
            if (!empty($token_customer)) {
                foreach ($token_customer as $tk) {
                    $token = $tk['device_token'];
                    if ($token != "") {
                        sendPushNotification($token, $notification_data_fields, IOS_BUNDLE_ID_CUSTOMER);
                    }
            ...
            # Insert this notification for user and merchant
            # Send to customer
            $insertData = ['title' => 'Order ' . $order_number_id . ' is customized', 'to_user_id' => $user_id, 'type' => 1, # Order related
            'order_id' => $selected_order_id, 'is_read' => 1, 'created_at' => time(), 'updated_at' => time(), ];
            $this->Common->insertData('notifications', $insertData);
            // echo $this->db->last_query();
            # Send to Merchant
            $insertData = ['title' => 'Order ' . $order_number_id . ' is customized', 'to_user_id' => $merchant_id, 'type' => 1, # Order related
            'order_id' => $selected_order_id, 'is_read' => 1, 'created_at' => time(), 'updated_at' => time(), ];
            $this->Common->insertData('notifications', $insertData);
            // echo $this->db->last_query();
    ...
    public function cms() {
        if ($this->id && $this->role == 1) # Only admin can do this
        {
            ...
            $pageData['cms_data'] = $this->Common->getData('cms', '*', 'id IN (1,2,6)'); //1 for Terms and conditions in table, //2 for privacy policy in table
            $this->load->view('masterpage', $pageData);
        } else {
            $this->load->view('login');
        }
    ...
    public function forgot_password_controller() {
        ...
        // $check_email = $this->Common->getData('users', 'id,email,role,fullname', "email='" . $email . "' AND role = 1  AND status NOT IN (2,3,4,5)");
    ...
    public function common_multiple_image_upload_function($uploaded_FILES_array, $upload_destiny) {
        ...
        foreach ($uploaded_FILES_array as $upload_file_name => $upload_file_value) {
            ...
            'allowed_types' => 'png|jpg|jpeg|mov',
            // 'max_size' => '1024',
    ...
    public function add_restaurant_controller() {
        ...
        if (!empty($check_exist_user_restaurant)) { //check IF ---START----
            ...
            if ($user_id != "" && $restaurant_name != "" && $business_type != "" && (($max_dinein_capacity != "" && $dinein_value == 1) || ($max_dinein_capacity == "" && $dinein_value == 0) || ($max_dinein_capacity != "" && $dinein_value == 0)) && (($food_type != "" && $business_type == 1) || ($food_type == "" && $business_type != 1)) && $description != "" && $postal_code != "" && $pin_address != "" && $unit_number != "" && $check_accept_type_select_or_not > 0 && $open_time != "" && $close_time != "" && $res_latitude != "" && $res_longtitude != "" && $delivery_handled_by != "") { //Form data is required IF---START---
                ...
                # Also if admin has created profile for any restaurant then we need to update the flag is_profile_completed in user table for this user id
                $this->Common->updateData('users',array('is_profile_completed' => 1),'id = "'.$user_id.'"');
                if ($insert_status > 0) {
                    # mail_send code start. This mail sends just a thankyou mail to merchant email Id
    ...
    public function add_edit_promotion_details($type) {
        ...
        public function edit_customer($user_id = '', $user_role = '') {
            if ($this->id && $this->role == 1) {
                $pageData['user_date'] = $this->Common->getData('users', 'id,role, fullname, email, mobile, profile_pic, user_pin_address,device_type,status,user_unit_number,user_street_address,user_postal_code,latitude,longitude,created_at', 'id = ' . $user_id . '  AND role = ' . $user_role . '  AND status != 5'); //role 3
        ...
        public function edit_customer_details() {
            ...
            $customer_street_address = $this->db->escape_str(trim($this->input->post('customer_street_address')));
            $postal_code = $this->db->escape_str(trim($this->input->post('postal_code')));
            $unit_number = $this->db->escape_str(trim($this->input->post('unit_number')));
            ...
            $get_user_data = $this->Common->getData("users", "fullname,email,mobile,user_pin_address,user_street_address,user_unit_number,user_postal_code,latitude,longitude", "id = " . $user_id . " and role = 3");
            $target = array(array('fullname' => $fullname, 'email' => $email, 'mobile' => $mobile, 'user_pin_address' => $customer_pin_address, 'user_street_address' => $customer_street_address, 'user_unit_number' => $unit_number, 'user_postal_code' => $postal_code, 'latitude' => $customer_latitude, 'longitude' => $customer_longtitude));
            if ($this->id && $this->role == 1) {
                //Check user, user's email and mobile no.  should be unique
                ...
                } else if (!empty($fullname) && !empty($email) && !empty($mobile) && $user_id != '' && $customer_pin_address != "" && $customer_street_address != "" && $unit_number != "" && $customer_latitude != "" && $customer_longtitude != "") {
                    $update_array = ['fullname' => $fullname, 'email' => $email, 'mobile' => $mobile, 'user_pin_address' => $customer_pin_address, 'user_street_address' => $customer_street_address, 'user_unit_number' => $unit_number, 'user_postal_code' => $postal_code, 'latitude' => $customer_latitude, 'longitude' => $customer_longtitude, 'updated_at' => time() ];
        ...
        public function products($table = '', $fromdate = 'all', $todate = 'all', $product_status = 'all', $product_food_type = 'all', $search_key = 'all', $restaurant_id = 0, $category_id = 0) {
            if ($this->id) {
                ...
                $common_query = "SELECT `products`.`id` as 'product_id',`products`.`product_name`,`products`.`price`,`products`.`product_image`,`products`.`short_desc`,`products`.`product_status`,`products`.`created_at`,`categories`.`category_status`,`categories`.`id` as 'category_id' FROM `products` INNER JOIN  `categories` ON `products`.`category_id` = `categories`.`id`  WHERE  `products`.`product_status` != 3  " . $query_part . "  ORDER BY `products`.`product_position_order` ASC ";
                ...
                $common_query_for_category = "SELECT  `id` as 'category_id',`category_name`,`category_status`,`category_image` FROM `categories` WHERE  `category_status` != 3  " . $query_part_for_category . "  ORDER BY `cat_position_order` ASC ";
                $page = ($this->uri->segment(11)) ? ($this->uri->segment(11) - 1) : 0;
                        ...
                        if (!empty($_FILES['product_image']['name'])) {
                            ...
                            if (!$this->upload->do_upload('product_image')) {
                                ...
                                if ($ext !== 'svg') {
                                    ...
                                    $source_image = $original_image_path . $fileName;
                                    $new_image = $resize_image_path . $fileName;
                                    $width = PRODUCT_IMAGE_RESIZE_WIDTH;
                                    $height = PRODUCT_IMAGE_RESIZE_HEIGHT;
                                    # Call this function to rezise the image and place in a new path
                                    $this->image_resize($source_image, $new_image, $width, $height);
                                    /*Image resize function ends here*/
                                } else {
        ...
        public function Update_CMS() {
            if ($this->id && $this->role == 1) # Only admin can do this
            {
                ...
                if ($page_name != "" && $page_key != "" && $page_primary_id != "" && $page_value != "") {
                    ...
                    // $page_value = str_replace('&nbsp;', ' ', $page_value);

                    $existing_image = $this->Common->getData('cms','page_image','page_key = "'.$page_key.'"');
                    $get_previous_popup_pic = $existing_image[0]['page_image'];
                    if($page_key == 'offer_popup' && !empty($_FILES['offer_popup_pic']))
                    {
                        ...
                        #delete previous profile image------- END -------
                        //upload image -------start
                        $exp = explode(".", $_FILES['offer_popup_pic']['name']);
                        $ext = end($exp);
                        $st1 = substr(date('ymd'), 0, 3);
                        $st2 = $st1 . rand(1, 100000);
                        $fileName = $st2 . time() . date('ymd') . "." . $ext;
                        $original_image_path = 'assets/images/ad_banners/'; //orignal image path
                        /* Image upload  */
                        $config['upload_path'] = $original_image_path;
                        $config['allowed_types'] = 'jpg|png|jpeg|bmp|svg';
                        $config['file_name'] = $fileName;
                        $this->upload->initialize($config);
                        if (!$this->upload->do_upload('offer_popup_pic')) {
                            $error_msg = $this->upload->display_errors();
                            $message = strip_tags($error_msg);
                            $this->session->set_flashdata('error', $message);
                            $page_image = $get_previous_popup_pic;
                        } else 
                        {
                            if (!empty($get_previous_popup_pic && file_exists($get_previous_popup_pic))) {
                                unlink($get_previous_popup_pic);
                            }
                            $page_image = $original_image_path.$fileName;
                        }
                    }else
                    {
                        $page_image = $get_previous_popup_pic;
                    ...
                    $update_array = ['page_name' => $this->input->post('page_name'), 'page_value' => $this->input->post('page_value'), 'page_image' => $page_image , 'updated_at' => time(), ];
        ...
        public function hear_about() {
            ...
            {
                $query = "SELECT `hear_about_us`.*,hear_about_us.id AS hear_abt_id , COUNT(users.hear_about_id) as fcount FROM `hear_about_us` LEFT JOIN users on users.hear_about_id = hear_about_us.id WHERE hear_about_us.status !=3 GROUP by hear_about_us.id";
                $pageData['hear_about_us'] = $this->Common->custom_query($query , 'get');
        ...
        public function update_outstanding_amount() {
            if ($this->id) {
                ...
                if ($edit_outstanding_amnt > 0) {
                    ...
                    $update_array = ['outstanding_amount' => $edit_outstanding_amnt, 'who_will_pay_outstanding_amount' => 0, 'is_paid_outstanding_amount' => 1, 'updated_at' => time(), 'paid_status' => 1];
                }
                # update data in orders table
                $update_status = $this->Common->updateData('orders', $update_array, "id = " . $selected_order_id);
                // echo $this->db->last_query();
        ...
        # This funtion is used to update the product category position used under Drag and Drop functionality
        public function update_category_product_poistion_order()
        {
            ...
            // print_r($_POST);
            $i=1;
            if($type == 1) # FOR CATEGORY POSITIONING
            {
                foreach($position as $k=>$v){
                    $this->Common->updateData('categories',array('cat_position_order' => $i),'id = "'.$v.'"');
                    $i++;  
                }
            }
            if($type == 2) # FOR PRODUCT POSITIONING
            {
                foreach($position as $k=>$v){
                    $this->Common->updateData('products',array('product_position_order' => $i),'id = "'.$v.'"');
                    $i++;  
                }   
            }
        }

        # This function is used to get the pending count of order and reservation
        public function get_order_dinein_status()
        {
            $total_pending_order = $this->Common->getData('orders', 'count(id) as total_pending_order', 'order_status = 0');
            $total_pending_order = $total_pending_order[0]['total_pending_order'];

            $total_pending_reservation = $this->Common->getData('table_reservations', 'count(id) as total_pending_reservation', 'is_accepted = 0');
            $total_pending_reservation = $total_pending_reservation[0]['total_pending_reservation'];
            
            $response = array('orders' => $total_pending_order , 'reservations' => $total_pending_reservation);
            echo json_encode($response);
        }

        # This function is used to delete the existing review
        public function delete_review(){
            if ($this->id) {
                $review_id = $this->db->escape_str(trim($this->input->post('review_id')));
                $this->Common->deleteData('ratings','id = "'.$review_id.'"');
                echo 1;
            } else {
                $this->load->view('login');
            }
