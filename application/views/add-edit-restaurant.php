...
{
    redirect(base_url('admin/add_edit_restaurant'));
}
else
{
        
  if (isset($restaurant_edit) && $restaurant_edit != "" && !empty($restaurant_edit)) {
        //print_r($restaurant_edit);
        
        $restaurant_id = $restaurant_edit[0]['id'];
        $merchant_fullname = $restaurant_edit[0]['fullname'];
        $rest_admin_email = $restaurant_edit[0]['email'];
        $rest_admin_contact_no = $restaurant_edit[0]['mobile'];
        $delivery_handled_by = $restaurant_edit[0]['delivery_handled_by'];//1 - restaurant 2 - By Kerala Eats
        $rest_pin_address = $restaurant_edit[0]['rest_pin_address'];
        $rest_pin_address = stripslashes($rest_pin_address);
        $rest_postal_code = $restaurant_edit[0]['rest_postal_code'];
        $rest_unit_number = $restaurant_edit[0]['rest_unit_number'];
        $rest_street_address = $restaurant_edit[0]['rest_street_address'];//restaurant address
        $rest_name = $restaurant_edit[0]['rest_name'];
        $rest_name = stripslashes($rest_name);
        $business_type = $restaurant_edit[0]['business_type'];
        $food_type = $restaurant_edit[0]['food_type'];//If restaurant's business type is food then this column will contain value. So value 0 (if not applicable) 1 (Restaurant) 2 (Kitchen)
        $is_veg = $restaurant_edit[0]['is_veg'];
        $res_admin_id = $restaurant_edit[0]['admin_id'];
        $res_description = strip_tags($restaurant_edit[0]['res_description']);
        $logo_image = $restaurant_edit[0]['logo_image'];
        $res_logo_image = trim(base_url().$restaurant_edit[0]['logo_image']);
        $banner_image = $restaurant_edit[0]['banner_image'];
        $res_banner_image = trim(base_url().$restaurant_edit[0]['banner_image']);
        $res_mobile_banner_image = trim(base_url().$restaurant_edit[0]['mobile_banner_image']);
        $mobile_banner_image = $restaurant_edit[0]['mobile_banner_image'];

        //Restaurant accept type
        $is_order_now_accept = $restaurant_edit[0]['is_order_now_accept'];
        $is_self_pickup_accept = $restaurant_edit[0]['is_self_pickup_accept'];
        $is_order_later_accept = $restaurant_edit[0]['is_order_later_accept'];
        $is_dinein_accept = $restaurant_edit[0]['is_dinein_accept'];
        $max_dinein_capacity = $restaurant_edit[0]['max_capacity'];

        //restauant open close time
        $open_time = $restaurant_edit[0]['open_time'];
        $close_time = $restaurant_edit[0]['close_time'];
        $break_start_time = $restaurant_edit[0]['break_start_time'];
        $break_end_time = $restaurant_edit[0]['break_end_time'];

        //offline online status
        $is_going_offline = $restaurant_edit[0]['is_going_offline'];

        #check toggle (enable disable)accroding exist value
        $offline_status = $restaurant_edit[0]['offline_status'];//1 - offline value exit in rest_offline  table, 0 - not exist
        $selected_offline_tag = $restaurant_edit[0]['selected_offline_tag'];//1 - offline value exit in rest_offline  table, 0 - not exist

        //restaurant document
        $uploaded_document = explode(',', $restaurant_edit[0]['uploaded_document']);
        $res_document = "";
        $count = 1;
        ...
        $rest_del_per_km_charge = $restaurant_edit[0]['per_km_charge'];// If delivery handled by restaurant then this will contain per kilo meter charge value  

        //commission setup
        $commission_type = $restaurant_edit[0]['commission_type'];
        $commission_value = trim($restaurant_edit[0]['commission_value']);

        $order_preparation_time = trim($restaurant_edit[0]['preparation_time']);
        $order_delivery_time = trim($restaurant_edit[0]['delivery_time']);

        $res_time_mode = $restaurant_edit[0]['time_mode'];//1 - for every day, 2 - for Specific day (if value 1 then open close and break time will be insert this table , if value 2 then open-close and break time will be insert in rest_time_daywise table )

         
         //For logo image---------
         if($logo_image != "" && empty($header['user_data'])){
              $res_logo_image = base_url().$restaurant_edit[0]['logo_image']; 
              $logo_image_exist = $logo_image; 
          }else{
              $res_logo_image = base_url('assets/images/default_restaurant_image.png'); 
              $logo_image_exist = "";
          }
          
          //banner image-----------
          if($banner_image != "" && empty($header['user_data'])){
              $res_banner_image = base_url().$restaurant_edit[0]['banner_image']; 
              $banner_image_exist = $banner_image; 
          }else{
              $res_banner_image = base_url('assets/images/default_restaurant_banner.jpg'); 
              $banner_image_exist = "";
          }

          //for mobile banner image-----------
          if($mobile_banner_image != "" && empty($header['user_data'])){
              $res_mobile_banner_image = base_url().$restaurant_edit[0]['mobile_banner_image']; 
              $mobile_banner_image_exist = $mobile_banner_image; 
          }else{
              $res_mobile_banner_image = base_url('assets/images/default_restaurant_banner.jpg'); 
              $mobile_banner_image_exist = "";
          }
      ...
      $is_veg = "";
      $res_description = "";
      $delivery_handled_by = 0;
      $max_dinein_capacity = "";

      $res_logo_image = base_url('assets/images/default_restaurant_image.png');
      $logo_image = base_url('assets/images/default_restaurant_image.png');
      $logo_image_exist = "";  

      $res_banner_image = base_url('assets/images/default_restaurant_banner.jpg');
      $banner_image_exist = "";
      $banner_image = base_url('assets/images/default_restaurant_banner.jpg');

      $res_mobile_banner_image = base_url('assets/images/default_restaurant_banner.jpg');
      $mobile_banner_image_exist = "";
      $mobile_banner_image = base_url('assets/images/default_restaurant_banner.jpg');
...
<?php
  ...
  $merchant_list = "";
  if (isset($user_name_list) && $user_name_list != "" && !empty($user_name_list)) {

      foreach ($user_name_list as $value) {
        $user_id = $value['id'];
        $merchant_name = $value['fullname'];

       
        if($user_id == $res_admin_id ){
              $select = 'selected';
        ...
        $merchant_list.= '<option value="'.$user_id.'" '.$select.'>'.$merchant_name.'</option>';
      }
  }
   ...
   <section class="section">
      ...
      <div class="add-restaurant">
         <div class="container">
            ...
            <div class="card-body">
               <form  autocomplete="off" enctype="multipart/form-data" id="res_form">
                  <div class="form-group">
                     <div class="row">
                        <div class="col-md-4 admin-profile-img">
                           <label style="font-size: 12px!important;
    font-weight: 800;">Logo  For Web and Mobile  (1000X350 px) <i class="fas fa-info-circle"
                              data-toggle="tooltip" data-placement="top"
                              title="Upload a square image that represents the business."
                              style="color: #666; font-size: 17px;"
                              ></i></label> 
                              <img id="disp_img" src="<?php echo $res_logo_image;?>"  alt="">
                               <div class="img-add">                                       
                                  <input type="file" id="file" name="logo_image" onchange="document.getElementById('disp_img').src = window.URL.createObjectURL(this.files[0])" accept="image/x-png,image/jpeg, image/jpg" required="" value="<?php echo $logo_image;?>">
                        ...
                        <div class="col-md-4 admin-profile-img">
                           <label  style="font-size: 12px!important;
    font-weight: 800;">Banner Image For Web (1920x360 px) <i class="fas fa-info-circle"
                              data-toggle="tooltip" data-placement="top"
                              title="Upload an image which represents the business. This will be displayed on the store home page."
                              style="color: #666; font-size: 17px;"
                              ></i></label>
                           <img id="disp_img2" src="<?php echo $res_banner_image; ?>"  alt="">
                           <div class="img-add">                                       
                              <input type="file" id="file2" name="banner_image" onchange="document.getElementById('disp_img2').src = window.URL.createObjectURL(this.files[0])" accept="image/x-png,image/jpeg, image/jpg" required="" value="<?php echo $banner_image;?>">
                              <label for="file2" ><i class="fas fa-plus"></i></label>
                           </div>
                           <span class="error" id="unfill_banner_image"></span>
                        </div>
                          <div class="col-md-4 admin-profile-img">
                           <label  style="font-size: 12px!important;
    font-weight: 800;">Banner Image For Mobile Web (768x384 px)
  <i class="fas fa-info-circle"
                              data-toggle="tooltip" data-placement="top"
                              title="Upload an image which represents the business. This will be displayed on the store home page."
                              style="color: #666; font-size: 17px;"
                              ></i></label>
                           <img id="disp_img3" src="<?php echo $res_mobile_banner_image; ?>"  alt="">
                           <div class="img-add">                                       
                              <input type="file" id="file3" name="mobile_banner_image" onchange="document.getElementById('disp_img3').src = window.URL.createObjectURL(this.files[0])" accept="image/x-png,image/jpeg, image/jpg" required="" value="<?php echo $mobile_banner_image;?>">
                              <label for="file3"><i class="fas fa-plus"></i></label>
                           </div>
                     ...
                     <div class="row">
                        <div class="col-md-6">
                           <label>Select Merchant Name*</label>
                            <select class="custom-select userstatus wv_filter_box_height form-control"  id="select_merchant" <?php echo  $edit_time_disable ;?>>
                                <option value="">Select Merchant Name</option>
                                <?php echo $merchant_list;?>
                            </select>
                            <span class="error" id="unfill_name"></span>
                        </div>
                        <div class="col-md-6">
                           <label>Email*</label>
                           <input type="email" name="email" placeholder="Enter email Address" value="<?php echo $rest_admin_email;?>" class="valid_email_des check_space"  id="res_email_valid" onblur="validateEmail(this,'#res_email_valid','#res_invalid_email')"  disabled = "disabled" style="cursor: not-allowed;" >
                  ...
                  <div class="form-group admin-input-field">
                     <div class="row">
                        <div class="col-md-6">
                           <label>Phone*</label>
                           <input type="tel" name="phone" placeholder="Enter phone number" value="<?php echo $rest_admin_contact_no;?>"  class="contact_number check_space" maxlength="9" minlength="8" id="res_phone" disabled = "disabled" style="cursor: not-allowed;" >
                            <span class="error" id="unfill_res_phone"></span>
                        </div>
                       
                        <div class="col-md-6">
                           <label>Postal code* </label>
                           <input type="text" maxlength="6" minlength="6" name="postal_code" class="check_space postal_code_length" id="postal_code" placeholder="Enter Postal Code" value="<?php echo $rest_postal_code;?>" required="">
                  ...
                  <div class="form-group admin-input-field">
                     <div class="row">
                        <div class="col-md-6">
                           ...
                           <input type="text" maxlength="100" minlength="10" name="phone" placeholder="Enter Street Address" value="<?php echo $rest_pin_address;?>"  class="check_space pin_address_length"  id="pin_address">
                            <span class="error" id="unfill_pin_address"></span>
                        </div>
                        
                        <div class="col-md-6">
                           <label>Unit Number*</label>
                           <input type="text" maxlength="10" minlength="5" name="unit_number" class="check_space pin_address_validation" id="unit_number" placeholder="Enter Unit Number" value="<?php echo $rest_unit_number;?>" required="">
                  ...
                  <div class="form-group admin-input-field">
                     <div class="row">
                        <!-- <div class="col-md-6">
                           <label>Street Address  </label>
                           <input type="text" maxlength="100" minlength="10" class="check_space" name="street_address" id="street_address" placeholder="Enter display address" value="<?php //echo $rest_street_address;?>" required="" >
                           <span class="error" id="unfill_street_address"></span>
                        </div> -->
                        <div class="col-md-6">
                           <label>Restaurant Name* </label>
                           <input type="text" maxlength="50" minlength="10"class="check_space" name="restaurant_name" placeholder="Enter restaurant name" value="<?php echo $rest_name;?>" id="res_name" required="">
                           <span class="error" id="unfill_res_name"></span>
                        </div>
                         <div class="col-md-6">
                           <label>Description (Max 360 Characters)</label>
                           <textarea maxlength="360" minlength="50" type="text" class="check_space description_length" name="description" placeholder="Description" value="<?php echo $res_description;?>" required="" id="res_description"><?php echo $res_description;?></textarea> 
                           <span class="error" id="invalid_description_length"></span>
                        </div>
                     </div>
                  </div>
                   <div class="form-group admin-input-field">
                   <div class="row">
                       <div class="col-md-6">
                        <label>Restaurant Accept Type*</label>
                        <label class="enabled-label" data-children-count="1">Order Now
                            <input type="checkbox" class="selected_rest_accept_type" id="order_now" name="selected_rest_accept_type" value="" <?php if($is_order_now_accept == 1){echo 'checked = ""';}?>>
                               <span class="checkmark_check"></span>
                         </label>
                          <label class="enabled-label" data-children-count="1">Self Pickup
                            <input type="checkbox" class="selected_rest_accept_type" id="self_pickup" name="selected_rest_accept_type" value=""  <?php if($is_self_pickup_accept == 1){echo 'checked = ""';}?>>
                               <span class="checkmark_check"></span>
                         </label>
                        <label class="enabled-label" data-children-count="1">Order For Later
                            <input type="checkbox" class="selected_rest_accept_type" id="order_later" name="selected_rest_accept_type" value=""  <?php if($is_order_later_accept == 1){echo 'checked = ""';}?>>
                               <span class="checkmark_check"></span>
                         </label>
                           <label class="enabled-label" data-children-count="1">Dine in
                            <input type="checkbox" class="selected_rest_accept_type" id="dinein" name="selected_rest_accept_type" value=""  <?php if($is_dinein_accept == 1){echo 'checked = ""';}?>>
                               <span class="checkmark_check"></span>
                         </label>
                         <span class="error" id="unselect_accept_type"></span>
                       </div>
                        <div class="col-sm-6">
                            <label>Who Will Handle The Delivery*</label>
                        <label class="enabled-label" data-children-count="1">By Restaruant
                            <input type="radio" class="" id="delivery_handled_by" name="delivery_handled_by" value="1" <?php if($delivery_handled_by  == 0 || $delivery_handled_by  == 1 ){echo 'checked = ""';}?>>
                               <span class="checkmark"></span>
                         </label>
                          <label class="enabled-label" data-children-count="1">By Kerala Eats
                            <input type="radio" class="" id="delivery_handled_by" name="delivery_handled_by" value="2"  <?php if($delivery_handled_by  == 2){echo 'checked = ""';}?>>
                               <span class="checkmark"></span>
                         </label>
                        </div>
                    </div><br>
                    <div class="row">
                          <div class="col-md-6">
                             ...
                             <input type="text" name="max_dinein_capacity" placeholder="Enter Max Capacity" value="<?php echo $max_dinein_capacity;?>"  class="postal_code_length check_space" id="max_dinein_capacity" maxlength="20" >
                              <span class="error" id="unfill_max_capacity"></span>
                          </div>
                         <?php if($this->uri->segment(3) == 1){?>
                      <div class="col-md-6">
                         <label>Restaurant Time (All Day)*</label><br>
                         <div class="row">
                            <div class="col-md-2">
                              All Day 
                            </div>
                  ...
                  <div class="form-group">
                    ...
                    <input type="hidden"  name="admin_id" id="admin_id" value="<?php echo $res_admin_id;?>">
                    <input type="hidden"  name="res_latitude" id="res_latitude" value="<?php //echo $res_admin_id;?>">
                    <input type="hidden"  name="res_longtitude" id="res_longtitude" value="<?php ///echo $res_admin_id;?>">
                     
                     <button type="button" id="get_res_lat_long" class="btn btn-primary change-password-btns mr-10">Save</button>
                     <button type="button" id="add_edit_res_submit" class="btn btn-primary change-password-btns mr-10 d-none">Save</button>
                     <a href="<?php echo base_url('admin/restaurant_list/');?>"  class="btn btn-secondary change-password_cancel">Cancel</a>
                  </div>
                   <hr>
               </form>
               <?php if($this->uri->segment(3) == 2){?><br>
               <div class="row">
                    <div class="col-md-3">
                        <label>Menu</label>
                    ...
                    <div class="col-md-4 text-center"><br>
                      <label>Is Receiving order (Offline/ Online Status)</label>
                       <label class="switch promocode-status" data-children-count="1">
                            <input type="checkbox" id="rest_going_online" <?php if($offline_status == 0){echo 'checked="checked"'; echo 'data-toggle="modal" data-target="#offline_online_restaurant_popup" data-backdrop="static" data-keyboard="false"'; echo ' value="1"';}else {echo ' value="0"';}?> selected_offline_tag="<?php $selected_offline_tag;?>">
                          ...
                          <table class="table">
                              <tbody class="cart-item-details">
                                   <tr>
                                      <th>All Day</th>
                                      <td>Open Time 
                                         <input type="time" class="form-control"  id="edit_open_time" value="<?php  echo "$open_time"; ?>"/>
                                          <span class="error" id="unfill_edit_open_time"></span>
                                      </td>
                                      <td>Close Time
                                          <input type="time" class="form-control" id="edit_close_time" name="close-time" placeholder="Select Time" required="" class="check_space"  value="<?php  echo "$close_time"; ?>">
                                          <span class="error" id="unfill_edit_close_time"></span>
                                      </td>
                                   </tr>
                                   <!-----Break on Mon--- START----->
                                    <tr id="break_time">
                                        <th>Break </th>
                                        <td>Start Time 
                                           <input type="time" class="form-control" id="edit_break_start_time" name="open-time" value="<?php  echo "$break_start_time"; ?>" required="" class="check_space"/>
                                          <span class="error" id="unfill_edit_break_start_time"></span>
                                        </td>
                                        <td>End Time
                                            <input type="time" id="edit_break_end_time" class="form-control" name="close-time" placeholder="Select Time" value="<?php  echo "$break_end_time"; ?>" required="" class="check_space"/>
                                             <span class="error" id="unfill_edit_break_end_time"></span>
                                        </td>
                                        <td>
                                        </td>
                                    </tr>
...
<script type="text/javascript">
   //Add Restaurant submit varification -----START-----
   
  var res_logo_image = '<?php echo $res_logo_image ; ?>';
  var exist_logo_image = '<?php echo $logo_image_exist ; ?>';
  var exist_banner_image = '<?php echo $banner_image_exist; ?>';
  var exist_mobile_banner_image = '<?php echo $mobile_banner_image_exist; ?>';
  var mode_type = '<?php echo $mode_type; ?>';
  //Add Restaurant submit varification -----END-----
</script>
<script src="https://maps.googleapis.com/maps/api/js?key= AIzaSyAZNYje6g5H5k6EiuMuFhd_gFm7DwloZLmuIv-I&sensor=false&libraries=places"></script>
