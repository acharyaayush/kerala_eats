...
//order single page -------EDIT Order update pick up Time-------START------------ 
 $('body').on('click', '#edit_pick_up_time_submit', function() {
     ...
     var edit_pick_up_from = $('#pick_up_from').val();
      ...
      if(edit_pick_up_from !="" && edit_pick_up_from != 0 && edit_pick_up_to !="" && edit_pick_up_to != 0){
            // Ajax-------SATRT------------
                $.ajax({
                url: BASE_URL+'admin/update_order_pick_up_time',
                data: { 
                    edit_pick_up_from:edit_pick_up_from,edit_pick_up_to:edit_pick_up_to,selected_order_id:order_id
                },
                type: 'post',
                success: function(response){
                   if(response == 1){
     ...
     var edit_track_link = $('#edit_track_link').val();
      ...
      if(edit_track_link !=""){
            // Ajax-------SATRT------------
                ...
                data: { 
                    edit_track_link:edit_track_link,selected_order_id:order_id
                },
                type: 'post',
                success: function(response){
                   if(response == 1){
...
/*function initMap() {
  const map = new google.maps.Map(document.getElementById("order_address_map"), {
    zoom: 8,
...
$("#d_address").keyup(function(e){
    ...
    function showPosition(position) {
        $('#delivery_latitude').val(position.coords.latitude);
        $('#delivery_latlong').val(position.coords.longitude);
      
    }

    // $('#delivery_latitude').val(d_address);
});*/
//ORDER detail Address map  ----auto complete---------------------END----------------------
