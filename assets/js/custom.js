...
// # This function will be called in every some seconds that will check whether any order or table reservation request is in pending status.

$(document).ready(function() {
    setInterval(function() {       
        $.ajax({
            url: BASE_URL+'admin/get_order_dinein_status/',
            data: { },
            type: 'post',
            ...
            // For orders
               ...
               {
                $("#update_blinker").html('<div class="blinking"><p class="circ"></p></div>');
               }else
               {
                $("#update_blinker").html('');
               }
           ...
           {
            $("#update_blinker_reservation").html('<div class="res_blinking"><p class="circ"></p></div>');
           }else
           {
            $("#update_blinker_reservation").html('');
           }
            },
        });
    }, 10000);
...
$(document).ready(function() {
    ...
    $('body').on('click', '#get_res_lat_long', function() {
        ...
        setTimeout(function() {
            $('#add_edit_res_submit').trigger('click');
        }, 1000);
    });
    ...
    $('body').on('click', '#add_edit_res_submit', function() {
        ...
        // pin_address = pin_address.replace(/'/g,"\'");
        // alert('pin'+pin_address);
        ...
        var max_dinein_capacity = $('#max_dinein_capacity').val();
        ...
        //if dine in is checked then max capacity is mandatory 
        if(jQuery.inArray("dinein", selected_rest_accept_type) === -1) {
            //console.log("Not exists in array");
            $('#max_dinein_capacity').css('border-color', '#ccc');
            $('#unfill_max_capacity').text('');
            var is_dinein_checked = false;
        } else {
            //console.log("Exists in array");
            if (max_dinein_capacity == "" || max_dinein_capacity == 0) {
                $('#max_dinein_capacity').css('border-color', 'red');
                $('#unfill_max_capacity').text('Please Fill Maximum Capacity');
            } else {
                $('#max_dinein_capacity').css('border-color', '#ccc');
                $('#unfill_max_capacity').text('');
            }
            var is_dinein_checked = true;
        }
        ...
        if (user_id != "" && res_phone != "" && res_email_valid != "" && res_name != "" && business_type != "" && ((max_dinein_capacity != "" && max_dinein_capacity != 0 && is_dinein_checked == true) || (max_dinein_capacity == "" && is_dinein_checked == false) || (max_dinein_capacity != "" && is_dinein_checked == false)) && ((food_type != "" && business_type == 1) || (food_type == "" && business_type != 1)) && res_description != "" && postal_code != "" && pin_address != "" && unit_number != "" && (upload_logo_image_status == true || exist_logo_image != "") && (upload_banner_image_status == true || exist_banner_image != "") && (upload_mobile_banner_image_status == true || exist_mobile_banner_image != "") && check_accept_type_select_or_not > 0 && delivery_handled_by != "" && res_open_time_fill_status == true && res_close_time_fill_status == true && logo_image_check_validation == true && banner_image_check_validation == true && mobile_banner_image_check_validation == true && res_latitude != "" && res_longtitude != "" && res_break_start_fill_status == true && res_break_end_fill_status == true) {
            ...
            fd.append('max_dinein_capacity', max_dinein_capacity); //if dine in is checked then value  will must go
...
$('body').on('change', '.res_status', function() {
    ...
    $.ajax({
        ...
        data: {
            ...
            enable_disable_status: active_status
        },
        type: 'post',
        success: function(response) {
            if (response == 1) {
...
$('body').on('click', '#rest_commission_submit', function() {
    ...
    if (rest_commission_type != "" && rest_commission_type != 0 && rest_commission_value != "" && rest_commission_value != 0) {
        ...
        $.ajax({
            ...
            data: {
                rest_commission_type: rest_commission_type,
                rest_commission_value: rest_commission_value,
...
$('body').on('click', '#rest_going_online', function() {
    ...
    //show  exist offline value for update --------------START-------------
    // future offline value exist in rest_offline table. then exist selected value will be show in modal when user click on toggle for going offline and it will be  update when user  save any  changes
    //HERE we delete exist value and do new entry with the updated new value
    $.ajax({
        url: BASE_URL + 'admin/GET_RestaurantOffline_Data/', //merchant detail
        data: {
        ...
        success: function(response) {
            if (response != 0) {
                var data = JSON.parse(response);
                if (data.offline_tag == 1) {
                    // we dont need to select hours becouse it will work for today date so 
                ...
                } else if (data.offline_tag == 2) {
                    $('.offline_day_btn_clr').trigger('click');
                    $('#days_offline_fromdate').val(data.offline_from);
                } else if (data.offline_tag == 3) {
                    $('.offline_multi_day_btn_clr').trigger('click');
                    $('#offline_rest_fromdate').val(data.offline_from);
                    $('#offline_rest_tilldate').val(data.offline_to);
                }
            }
            ...
            if (response == 0) {
                ...
                $('#offline_rest_fromdate').val('');
                $('#offline_rest_tilldate').val('');
            }
        },

    });
    //show  exist offline value for update --------------END-------------
...
$('body').on('click', '#rest_online_offline_save', function() {
    ...
    // alert("offline_tag"+offline_tag);
    // alert("offline_type"+offline_type);
    // alert("selected_restaurant_id"+selected_restaurant_id);
    ...
    else if (offline_tag == 4) { // # UNLIMITED_OFFLINE
        //required fields--------------end-----------
        // alert("IN UNLIMITED");
        offline_tag_value = 4;
    ...
    if (selected_restaurant_id != "" && offline_type != "" && offline_tag != "" && offline_tag_value != "") {
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response == 2) {
                    // date selected for future date not for today means toggle will be visbile enable till given date
                    $('#rest_going_online').attr('data-target', '#offline_online_restaurant_popup');
                    $('#rest_going_online').attr('data-toggle', 'modal');
                    $('#rest_going_online').val(1);
                    $('#rest_going_online').prop('checked', true);
...
function common_edit_break_start_time_check(break_start_time, open_time, break_end_time, close_time) {
    // alert("break_end_time"+break_end_time);
    if (break_start_time >= open_time && break_start_time <= close_time) {
        // alert(' between');
        if(break_end_time != '')
        {
            if (break_start_time > break_end_time) {
                ...
                $('#unfill_edit_break_start_time').text('Time should be smallerrrrr');
            } else {
        ...
        }else
        {
            ...
            $('#unfill_edit_break_end_time').text('Please Select End Time');
        }
    } else {
        // alert('not between');
        edit_break_time_validate_status = false;
    ...
    // alert('edit_break_time_validate_status'+edit_break_time_validate_status);
...
$('body').on('click', '#rest_open_close_time_break_submit', function() {
    ...
    if (rest_time_mode == 2) { // 1 every day, 2 - specific day
        ...
        if (mon_open_time == "" && $('#mon_close_status').prop("checked") == false) {
            ...
            if ($('#mon_close_status').prop("checked") == false && mon_open_time >= mon_close_time && mon_open_time >= '00:00' && mon_open_time <= '23:59') {
                $('#mon_open_time').css('border-color', 'red');
                $('#unfill_mon_open_time').text('Open Time Should Be Smaller');
                final_filled_status.push(false);
            } else {
                $('#mon_open_time').css('border-color', '#ccc');
                $('#unfill_mon_open_time').text('');
        ...
        if (mon_close_time == "" && $('#mon_close_status').prop("checked") == false) {
            ...
            if ($('#mon_close_status').prop("checked") == false && mon_open_time >= mon_close_time && mon_open_time >= '00:00' && mon_open_time <= '23:59') {
                $('#mon_close_time').css('border-color', 'red');
                $('#unfill_mon_close_time').text('Close  Time Should Be Greater');
                final_filled_status.push(false);
            } else {
                $('#mon_close_time').css('border-color', '#ccc');
                $('#unfill_mon_close_time').text('');
        ...
        if (tue_open_time == "" && $('#tue_close_status').prop("checked") == false) {
            ...
            if ($('#tue_close_status').prop("checked") == false && tue_open_time >= tue_close_time && tue_open_time >= '00:00' && tue_open_time <= '23:59') {
                $('#tue_open_time').css('border-color', 'red');
                $('#unfill_tue_open_time').text('Open Time Should Be Smaller');
                final_filled_status.push(false);
            } else {
                $('#tue_open_time').css('border-color', '#ccc');
                $('#unfill_tue_open_time').text('');
        ...
        if (tue_close_time == "" && $('#tue_close_status').prop("checked") == false) {
            ...
            if ($('#tue_close_status').prop("checked") == false && tue_open_time >= tue_close_time && tue_open_time >= '00:00' && tue_open_time <= '23:59') {
                $('#tue_close_time').css('border-color', 'red');
                $('#unfill_tue_close_time').text('Close  Time Should Be Greater');
                final_filled_status.push(false);
            } else {
                $('#tue_close_time').css('border-color', '#ccc');
                $('#unfill_tue_close_time').text('');
        ...
        if (wed_open_time == "" && $('#wed_close_status').prop("checked") == false) {
            ...
            if ($('#wed_close_status').prop("checked") == false && wed_open_time >= wed_close_time && wed_open_time >= '00:00' && wed_open_time <= '23:59') {
                $('#wed_open_time').css('border-color', 'red');
                $('#unfill_wed_open_time').text('Open Time Should Be Smaller');
                final_filled_status.push(false);
            } else {
                $('#wed_open_time').css('border-color', '#ccc');
                $('#unfill_wed_open_time').text('');
        ...
        if (wed_close_time == "" && $('#wed_close_status').prop("checked") == false) {
            ...
            if ($('#wed_close_status').prop("checked") == false && wed_open_time >= wed_close_time && wed_open_time >= '00:00' && wed_open_time <= '23:59') {
                $('#wed_close_time').css('border-color', 'red');
                $('#unfill_wed_close_time').text('Close  Time Should Be Greater');
                final_filled_status.push(false);
            } else {
                $('#wed_close_time').css('border-color', '#ccc');
                $('#unfill_wed_close_time').text('');
        ...
        if (thu_open_time == "" && $('#thu_close_status').prop("checked") == false) {
            ...
            if ($('#thu_close_status').prop("checked") == false && thu_open_time >= thu_close_time && thu_open_time >= '00:00' && thu_open_time <= '23:59') {
                $('#thu_open_time').css('border-color', 'red');
                $('#unfill_thu_open_time').text('Open Time Should Be Smaller');
                final_filled_status.push(false);
            } else {
                $('#thu_open_time').css('border-color', '#ccc');
                $('#unfill_thu_open_time').text('');
        ...
        if (thu_close_time == "" && $('#thu_close_status').prop("checked") == false) {
            ...
            if ($('#thu_close_status').prop("checked") == false && thu_open_time >= thu_close_time && thu_open_time >= '00:00' && thu_open_time <= '23:59') {
                $('#thu_close_time').css('border-color', 'red');
                $('#unfill_thu_close_time').text('Close  Time Should Be Greater');
                final_filled_status.push(false);
            } else {
                $('#thu_close_time').css('border-color', '#ccc');
                $('#unfill_thu_close_time').text('');
        ...
        if (fri_open_time == "" && $('#fri_close_status').prop("checked") == false) {
            ...
            if ($('#fri_close_status').prop("checked") == false && fri_open_time >= fri_close_time && fri_open_time >= '00:00' && fri_open_time <= '23:59') {
                $('#fri_open_time').css('border-color', 'red');
                $('#unfill_fri_open_time').text('Open Time Should Be Smaller');
                final_filled_status.push(false);
            } else {
                $('#fri_open_time').css('border-color', '#ccc');
                $('#unfill_fri_open_time').text('');
        ...
        if (fri_close_time == "" && $('#fri_close_status').prop("checked") == false) {
            ...
            if ($('#fri_close_status').prop("checked") == false && fri_open_time >= fri_close_time && fri_open_time >= '00:00' && fri_open_time <= '23:59') {
                $('#fri_close_time').css('border-color', 'red');
                $('#unfill_fri_close_time').text('Close  Time Should Be Greater');
                final_filled_status.push(false);
            } else {
                $('#fri_close_time').css('border-color', '#ccc');
                $('#unfill_fri_close_time').text('');
        ...
        if (sat_open_time == "" && $('#sat_close_status').prop("checked") == false) {
            ...
            if ($('#sat_close_status').prop("checked") == false && sat_open_time >= sat_close_time && sat_open_time >= '00:00' && sat_open_time <= '23:59') {
                $('#sat_open_time').css('border-color', 'red');
                $('#unfill_sat_open_time').text('Open Time Should Be Smaller');
                final_filled_status.push(false);
            } else {
                $('#sat_open_time').css('border-color', '#ccc');
                $('#unfill_sat_open_time').text('');
        ...
        if (sat_close_time == "" && $('#sat_close_status').prop("checked") == false) {
            ...
            if ($('#sat_close_status').prop("checked") == false && sat_open_time >= sat_close_time && sat_open_time >= '00:00' && sat_open_time <= '23:59') {
                $('#sat_close_time').css('border-color', 'red');
                $('#unfill_sat_close_time').text('Close  Time Should Be Greater');
                final_filled_status.push(false);
            } else {
                $('#sat_close_time').css('border-color', '#ccc');
                $('#unfill_sat_close_time').text('');
        ...
        if (sun_open_time == "" && $('#sun_close_status').prop("checked") == false) {
            ...
            if ($('#sun_close_status').prop("checked") == false && sun_open_time >= sun_close_time && sun_open_time >= '00:00' && sun_open_time <= '23:59') {
                $('#sun_open_time').css('border-color', 'red');
                $('#unfill_sun_open_time').text('Open Time Should Be Smaller');
                final_filled_status.push(false);
            } else {
                $('#sun_open_time').css('border-color', '#ccc');
                $('#unfill_sun_open_time').text('');
        ...
        if (sun_close_time == "" && $('#sun_close_status').prop("checked") == false) {
            ...
            if ($('#sun_close_status').prop("checked") == false && sun_open_time >= sun_close_time && sun_open_time >= '00:00' && sun_open_time <= '23:59') {
                $('#sun_close_time').css('border-color', 'red');
                $('#unfill_sun_close_time').text('Close  Time Should Be Greater');
                final_filled_status.push(false);
            } else {
                $('#sun_close_time').css('border-color', '#ccc');
                $('#unfill_sun_close_time').text('');
        ...
        if (mon_break_time_mode == 1) {
            if (mon_break_start_time == "") {
                ...
                if ((mon_open_time == mon_break_start_time || mon_close_time == mon_break_start_time) || (mon_open_time > mon_break_start_time || mon_close_time < mon_break_end_time) || (mon_close_time < mon_break_start_time)) {
                    $('#mon_break_start_time').css('border-color', 'red');
                    $('#unfill_mon_break_start_time').text('Time should be between open and close time');
                    final_filled_status.push(false);
                } else if (mon_break_start_time >= mon_break_end_time && mon_break_start_time >= '00:00' && mon_break_start_time <= '23:59') {
                    $('#mon_break_start_time').css('border-color', 'red');
                    $('#unfill_mon_break_start_time').text('Start Time Should Be Smaller');
                    final_filled_status.push(false);
                } else {
            ...
            if (mon_break_end_time == "") {
                ...
                if ((mon_open_time == mon_break_start_time || mon_close_time == mon_break_start_time) || (mon_open_time > mon_break_start_time || mon_close_time < mon_break_end_time) || (mon_close_time < mon_break_start_time)) {
                    $('#mon_break_end_time').css('border-color', 'red');
                    $('#unfill_mon_break_end_time').text('Time should be between open and close time');
                    final_filled_status.push(false);
                } else if (mon_break_start_time >= mon_break_end_time && mon_break_start_time >= '00:00' && mon_break_start_time <= '23:59') {
                    $('#mon_break_end_time').css('border-color', 'red');
                    $('#unfill_mon_break_end_time').text('End Time Should Be Greater');
                    final_filled_status.push(false);
                } else {
                    $('#mon_break_end_time').css('border-color', '#ccc');
                    $('#unfill_mon_break_end_time').text('');
        ...
        if (tue_break_time_mode == 1) {
            if (tue_break_start_time == "") {
                ...
                if ((tue_open_time == tue_break_start_time || tue_close_time == tue_break_start_time) || (tue_open_time > tue_break_start_time || tue_close_time < tue_break_end_time) || (tue_close_time < tue_break_start_time)) {
                    $('#tue_break_start_time').css('border-color', 'red');
                    $('#unfill_tue_break_start_time').text('Time should be between open and close time');
                    final_filled_status.push(false);
                } else if (tue_break_start_time >= tue_break_end_time && tue_break_start_time >= '00:00' && tue_break_start_time <= '23:59') {
                    $('#tue_break_start_time').css('border-color', 'red');
                    $('#unfill_tue_break_start_time').text('Start Time Should Be Smaller');
                    final_filled_status.push(false);
                } else {
            ...
            if (tue_break_end_time == "") {
                ...
                if ((tue_open_time == tue_break_start_time || tue_close_time == tue_break_start_time) || (tue_open_time > tue_break_start_time || tue_close_time < tue_break_end_time) || (tue_close_time < tue_break_start_time)) {
                    $('#tue_break_end_time').css('border-color', 'red');
                    $('#unfill_tue_break_end_time').text('Time should be between open and close time');
                    final_filled_status.push(false);
                } else if (tue_break_start_time >= tue_break_end_time && tue_break_start_time >= '00:00' && tue_break_start_time <= '23:59') {
                    $('#tue_break_end_time').css('border-color', 'red');
                    $('#unfill_tue_break_end_time').text('End Time Should Be Greater');
                    final_filled_status.push(false);
                } else {
                    $('#tue_break_end_time').css('border-color', '#ccc');
                    $('#unfill_tue_break_end_time').text('');
        ...
        if (wed_break_time_mode == 1) {
            if (wed_break_start_time == "") {
                ...
                if ((wed_open_time == wed_break_start_time || wed_close_time == wed_break_start_time) || (wed_open_time > wed_break_start_time || wed_close_time < wed_break_end_time) || (wed_close_time < wed_break_start_time)) {
                    $('#wed_break_start_time').css('border-color', 'red');
                    $('#unfill_wed_break_start_time').text('Time should be between open and close time');
                    final_filled_status.push(false);
                } else if (wed_break_start_time >= wed_break_end_time && wed_break_start_time >= '00:00' && wed_break_start_time <= '23:59') {
                    $('#wed_break_start_time').css('border-color', 'red');
                    $('#unfill_wed_break_start_time').text('Start Time Should Be Smaller');
                    final_filled_status.push(false);
                } else {
            ...
            if (wed_break_end_time == "") {
                ...
                if ((wed_open_time == wed_break_start_time || wed_close_time == wed_break_start_time) || (wed_open_time > wed_break_start_time || wed_close_time < wed_break_end_time) || (wed_close_time < wed_break_start_time)) {
                    $('#wed_break_end_time').css('border-color', 'red');
                    $('#unfill_wed_break_end_time').text('Time should be between open and close time');
                    final_filled_status.push(false);
                } else if (wed_break_start_time >= wed_break_end_time && wed_break_start_time >= '00:00' && wed_break_start_time <= '23:59') {
                    $('#wed_break_end_time').css('border-color', 'red');
                    $('#unfill_wed_break_end_time').text('End Time Should Be Greater');
                    final_filled_status.push(false);
                } else {
                    $('#wed_break_end_time').css('border-color', '#ccc');
                    $('#unfill_wed_break_end_time').text('');
        ...
        if (thu_break_time_mode == 1) {
            if (thu_break_start_time == "") {
                ...
                if ((thu_open_time == thu_break_start_time || thu_close_time == thu_break_start_time) || (thu_open_time > thu_break_start_time || thu_close_time < thu_break_end_time) || (thu_close_time < thu_break_start_time)) {
                    $('#thu_break_start_time').css('border-color', 'red');
                    $('#unfill_thu_break_start_time').text('Time should be between open and close time');
                    final_filled_status.push(false);
                } else if (thu_break_start_time >= thu_break_end_time && thu_break_start_time >= '00:00' && thu_break_start_time <= '23:59') {
                    $('#thu_break_start_time').css('border-color', 'red');
                    $('#unfill_thu_break_start_time').text('Start Time Should Be Smaller');
                    final_filled_status.push(false);
                } else {
            ...
            if (thu_break_end_time == "") {
                ...
                if ((thu_open_time == thu_break_start_time || thu_close_time == thu_break_start_time) || (thu_open_time > thu_break_start_time || thu_close_time < thu_break_end_time) || (thu_close_time < thu_break_start_time)) {
                    $('#thu_break_end_time').css('border-color', 'red');
                    $('#unfill_thu_break_end_time').text('Time should be between open and close time');
                    final_filled_status.push(false);
                } else if (thu_break_start_time >= thu_break_end_time && thu_break_start_time >= '00:00' && thu_break_start_time <= '23:59') {
                    $('#thu_break_end_time').css('border-color', 'red');
                    $('#unfill_thu_break_end_time').text('End Time Should Be Greater');
                    final_filled_status.push(false);
                } else {
                    $('#thu_break_end_time').css('border-color', '#ccc');
                    $('#unfill_thu_break_end_time').text('');
        ...
        if (fri_break_time_mode == 1) {
            if (fri_break_start_time == "") {
                ...
                if ((fri_open_time == fri_break_start_time || fri_close_time == fri_break_start_time) || (fri_open_time > fri_break_start_time || fri_close_time < fri_break_end_time) || (fri_close_time < fri_break_start_time)) {
                    $('#fri_break_start_time').css('border-color', 'red');
                    $('#unfill_fri_break_start_time').text('Time should be between open and close time');
                    final_filled_status.push(false);
                } else if (fri_break_start_time >= fri_break_end_time && fri_break_start_time >= '00:00' && fri_break_start_time <= '23:59') {
                    $('#fri_break_start_time').css('border-color', 'red');
                    $('#unfill_fri_break_start_time').text('Start Time Should Be Smaller');
                    final_filled_status.push(false);
                } else {
            ...
            if (fri_break_end_time == "") {
                ...
                if ((fri_open_time == fri_break_start_time || fri_close_time == fri_break_start_time) || (fri_open_time > fri_break_start_time || fri_close_time < fri_break_end_time) || (fri_close_time < fri_break_start_time)) {
                    $('#fri_break_end_time').css('border-color', 'red');
                    $('#unfill_fri_break_end_time').text('Time should be between open and close time');
                    final_filled_status.push(false);
                } else if (fri_break_start_time >= fri_break_end_time && fri_break_start_time >= '00:00' && fri_break_start_time <= '23:59') {
                    $('#fri_break_end_time').css('border-color', 'red');
                    $('#unfill_fri_break_end_time').text('End Time Should Be Greater');
                    final_filled_status.push(false);
                } else {
                    $('#fri_break_end_time').css('border-color', '#ccc');
                    $('#unfill_fri_break_end_time').text('');
        ...
        if (sat_break_time_mode == 1) {
            if (sat_break_start_time == "") {
                ...
                if ((sat_open_time == sat_break_start_time || sat_close_time == sat_break_start_time) || (sat_open_time > sat_break_start_time || sat_close_time < sat_break_end_time) || (sat_close_time < sat_break_start_time)) {
                    $('#sat_break_start_time').css('border-color', 'red');
                    $('#unfill_sat_break_start_time').text('Time should be between open and close time');
                    final_filled_status.push(false);
                } else if (sat_break_start_time >= sat_break_end_time && sat_break_start_time >= '00:00' && sat_break_start_time <= '23:59') {
                    $('#sat_break_start_time').css('border-color', 'red');
                    $('#unfill_sat_break_start_time').text('Start Time Should Be Smaller');
                    final_filled_status.push(false);
                } else {
                    $('#sat_break_start_time').css('border-color', '#ccc');
                    $('#unfill_sat_break_start_time').text('');
            ...
            if (sat_break_end_time == "") {
                ...
                if ((sat_open_time == sat_break_start_time || sat_close_time == sat_break_start_time) || (sat_open_time > sat_break_start_time || sat_close_time < sat_break_end_time) || (sat_close_time < sat_break_start_time)) {
                    $('#sat_break_end_time').css('border-color', 'red');
                    $('#unfill_sat_break_end_time').text('Time should be between open and close time');
                    final_filled_status.push(false);
                } else if (sat_break_start_time >= sat_break_end_time && sat_break_start_time >= '00:00' && sat_break_start_time <= '23:59') {
                    $('#sat_break_end_time').css('border-color', 'red');
                    $('#unfill_sat_break_end_time').text('End Time Should Be Greater');
                    final_filled_status.push(false);
                } else {
                    $('#sat_break_end_time').css('border-color', '#ccc');
                    $('#unfill_sat_break_end_time').text('');
        ...
        if (sun_break_time_mode == 1) {
            if (sun_break_start_time == "") {
                ...
                if ((sun_open_time == sun_break_start_time || sun_close_time == sun_break_start_time) || (sun_open_time > sun_break_start_time || sun_close_time < sun_break_end_time) || (sun_close_time < sun_break_start_time)) {
                    $('#sun_break_start_time').css('border-color', 'red');
                    $('#unfill_sun_break_start_time').text('Time should be between open and close time');
                    final_filled_status.push(false);
                } else if (sun_break_start_time >= sun_break_end_time && sun_break_start_time >= '00:00' && sun_break_start_time <= '23:59') {
                    $('#sun_break_start_time').css('border-color', 'red');
                    $('#unfill_sun_break_start_time').text('Start Time Should Be Smaller');
                    final_filled_status.push(false);
                } else {
            ...
            if (sun_break_end_time == "") {
                ...
                if ((sun_open_time == sun_break_start_time || sun_close_time == sun_break_start_time) || (sun_open_time > sun_break_start_time || sun_close_time < sun_break_end_time) || (sun_close_time < sun_break_start_time)) {
                    $('#sun_break_end_time').css('border-color', 'red');
                    $('#unfill_sun_break_end_time').text('Time should be between open and close time');
                    final_filled_status.push(false);
                } else if (sun_break_start_time >= sun_break_end_time && sun_break_start_time >= '00:00' && sun_break_start_time <= '23:59') {
                    $('#sun_break_end_time').css('border-color', 'red');
                    $('#unfill_sun_break_end_time').text('End Time Should Be Greater');
                    final_filled_status.push(false);
                } else {
                    $('#sun_break_end_time').css('border-color', '#ccc');
                    $('#unfill_sun_break_end_time').text('');
        ...
        // alert("qqqqqqq"+mon_close_status);
...
$('body').on('click', '#rest_delivery_per_km_charge_submit', function() {
    ...
    if (delivery_per_km_charge != "") { //&& delivery_per_km_charge != 0
        ...
        $.ajax({
            ...
            data: {
                delivery_per_km_charge: delivery_per_km_charge,
                selected_restaurant_id: selected_restaurant_id
            },
            type: 'post',
            success: function(response) {
...
$('body').on('click', '#rest_order_prepration_and_delivery_time_submit', function() {
    ...
    if (rest_order_preparation_time != "" && rest_order_preparation_time != 0 && rest_order_delivery_time && rest_order_delivery_time != 0) {
        ...
        $.ajax({
            ...
            data: {
                rest_order_preparation_time: rest_order_preparation_time,
                rest_order_delivery_time: rest_order_delivery_time,
                selected_restaurant_id: selected_restaurant_id
            },
            type: 'post',
            success: function(response) {
...
$('body').on('change', '.change_hear_abt_status', function() {
    var hear_abt_id = $(this).attr('id');
    var status = $(this).attr('name');
    $.ajax({
        url: BASE_URL + 'admin/enable_disable_hear_abt_status/',
        data: {
            hear_abt_id: hear_abt_id,
            status: status
        },
        type: 'post',
        success: function() {
...
$(document).ready(function() {
    ...
    $('#SelectRestaurantDropdown').addClass('show');
    ...
    function common_getting_restauant_product_or_category_id_by_promo_code_name(selected_restaurant_id, applied_on) {
        ...
        $.ajax({
            ...
            data: {
                promotion_code_name: edit_promotion_code_name,
                checked_restaurant_id: selected_restaurant_id,
                level_id: applied_on
            },
            type: 'post',
            success: function(response) {
                if (response != 0) {
                    ...
                    $.each(json, function(index, json) {
                        ...
                        if (applied_on == 1 || applied_on == 2) { //restaurant and delivery dropdown 

                            if (json.restaurant_id != 0) {
                                //selected restaurant id will be select only
                                $('#restaurant_id_' + json.restaurant_id).attr('checked', 'checked');
                            } else if (json.if_promo_for_all_rest == 1) {
                                //select all restaurant id 
    ...
    $('body').on('change', '#applied_on,.selected_restaurant_id', function() {
        ...
        if (promotion_mode_type == 2 && edit_promotion_code_name != "") {
            ...
            //if all select and user want to uncheck some restaturant then all(checkbox tab) will be uncheck because in the all condtion only one entry will be insert in db 
            var check_if_click_all = $(this).attr('id');
            if (check_if_click_all != 'select_all_restaurant') {
                $('#select_all_restaurant').prop('checked', false);
            }

            if ($('.selected_restaurant_id').is(':checkbox')) {
                $('.selected_restaurant_id').removeAttr('checked');
            }
            if ($('.selected_restaurant_id').is(':radio')) {
                $('.selected_restaurant_id:last').prop('checked', false);
            }
...
// Check uncheck if promo valid or forever // 
// var is_forever;
// alert('is_forever'+is_forever);
var cur_use_type = $("#cur_use_type").val();
// alert("cur_use_type"+cur_use_type);
if (cur_use_type == 1) { // # Its a forever promotion
    $('.forever_promo').trigger('click');
} else {
    $('.limited_promo').trigger('click');
...
$('body').on('click', '#add_promotion_submit', function() {
    ...
    var promo_use_type = $('#promo_use_type:checked').val();
    // alert("promo_use_type"+promo_use_type);
    ...
    if (PromotionFormData.selected_restaurant_id == '' && (PromotionFormData.applied_on == '1' || PromotionFormData.applied_on == '2' || PromotionFormData.applied_on == '3' || PromotionFormData.applied_on == '4' || PromotionFormData.applied_on == '8')) {
        $('#SelectRestaurantInput,#SelectRestaurantDropdown').css('border-color', 'red');
        $('#unselect_restaurant').text('Please Select Restaurant');
        check_PromotionFormData_is_blank.push(false);
    } else {
...
$('body').on('change', '.discount_status', function() {
    ...
    $.ajax({
        ...
        data: {
            ...
            discount_status_value: discount_status_value
        },
        type: 'post',
        success: function(response) {
            if (response == 1) {
...
$('body').on('click', '.add_edit_ad_banner_popup', function() {
    ...
    if (mode == 1) { // add
        ...
        $.ajax({
            ...
            type: 'post',
            data: {
                ad_banner_id: edit_ad_banner_id
            },
            success: function(response) {
...
// Function to delete review by Super admin Start
$('body').on('click', '.review_delete', function() {

    var review_input_id = $(this).attr('id'); //input id
    var review_id_ar = review_input_id.split("_");
    var review_id = review_id_ar[1];
...
// Function to delete review by Super admin End
...
$(".search_key").keydown(function(e) { //on enter
    if (e.keyCode === 13) {
        $('#search_user_wallet_list_data').trigger("click");
    }
});

$('body').on('click', '#search_user_wallet_list_data', function() {
    var from_date = $('#fromdate').val();
    var to_date = $('#todate').val();
    var type = $('.wallet_type').val();
    var user_id = $('#wallet_user_id').val();
    ...
    if (from_date != "all" || to_date != "all" || type != "all") {

        window.location.replace(BASE_URL + 'admin/wallet_history/' + user_id + '/' + from_date + '/' + to_date + '/' + type + '/'); //load only table on a promo_code_taurant page 
    } else {
        window.location.replace(BASE_URL + 'admin/wallet_history/' + user_id + '/');
    }
});
...
$('body').on('click', '#search_reservation', function() {
    // alert('1111111111111');
    var from_date = $('#fromdate').val();
    var to_date = $('#todate').val();
    var accept_status = $('#accept_status').val();
    ...
    if (from_date != "all" || to_date != "all" || accept_status != "all" || restaurant_id != 0) {

        window.location.replace(BASE_URL + 'admin/reservations/' + from_date + '/' + to_date + '/' + accept_status + '/' + restaurant_id + '/');
    } else {
        window.location.replace(BASE_URL + 'admin/reservations/');
    }
});

$('body').on('click', '#filter_review', function() {
    
    var restaurant_id = $("#restaurant_id").val();

    if (restaurant_id != 0) {
        window.location.replace(BASE_URL + 'admin/review_listnig/'+restaurant_id + '/');
    } else {
        window.location.replace(BASE_URL + 'admin/review_listnig/0/');
    }
});
...
$('body').on('click', '#get_customer_lat_long', function() {
    ...
    getLocation(pin_address);
    setTimeout(function() {
        $('#customer_edit_submit').trigger('click');
    }, 1000);
});
...
$('body').on('click', '#customer_edit_submit', function() {
    ...
    if (customer_street_address == "") {
        $('#customer_street_address').css('border-color', 'red');
        $('#unfill_street_address').text('Please Fill BLK /House /Apartment No');
    } else {
        $('#customer_street_address').css('border-color', '#ccc');
    ...
    // Check file selected or not
    if (fullname != "" && email != "" && mobile != "" && email_status == true && customer_pin_address != "" && customer_street_address != "" && unit_number != "" && postal_code != "" && customer_latitude != "" && customer_longtitude != "") {
        ...
        $.ajax({
            ...
            data: {
                ...
                mobile: mobile,
                user_id: user_id,
                customer_pin_address: customer_pin_address,
                customer_street_address: customer_street_address,
                unit_number: unit_number,
                postal_code: postal_code,
                customer_latitude: customer_latitude,
                customer_longtitude: customer_longtitude
            },
...
$('body').on('click', '#customer_credit_amount_submit', function() {
    ...
    var customer_credit_comments = $("#add_wallet_comments").val();

    if (customer_credit_comments == '') {
        customer_credit_comments = 'NA';
    }
    ...
    if (customer_id != "" && customer_credit_amount != "") {
        ...
        $.ajax({
            ...
            data: {
                ...
                customer_credit_comments: customer_credit_comments
            },
...
$('body').on('click', '#customer_debit_amount_submit', function() {
    ...
    var customer_debit_amount_comments = $("#deduct_wallet_comments").val();
    if (customer_debit_amount_comments == '') {
        customer_debit_amount_comments = 'NA';
    }
    ...
    if (customer_id != "" && customer_debit_amount != "") {
        ...
        $.ajax({
            ...
            data: {
                customer_id: customer_id,
                customer_debit_amount: customer_debit_amount,
                customer_debit_amount_comments: customer_debit_amount_comments
            },
...
$(document).ready(function() {
    ...
    $('body').on('change', '.product_status', function() {
        ...
        var selected_offline_tag = $(this).attr('selected_offline_tag');
        $('#offline_tag').val(selected_offline_tag);
        if (selected_offline_tag == 1) {
            $('.offline_hour_btn_clr').trigger('click');
        } else if (selected_offline_tag == 2) {
            $('.offline_day_btn_clr').trigger('click');
        } else if (selected_offline_tag == 3) {
            $('.offline_multi_day_btn_clr').trigger('click');
        }
        ...
        // alert(selected_offline_tag);
        $.ajax({
            url: BASE_URL + 'admin/GET_ProductOffline_Data/', //Product detail
            data: {
            ...
            success: function(response) {
                if (response != 0) {
                    ...
                    if (selected_offline_tag == 1) {
                        // we dont need to select hours becouse it will work for today date so 
                        // Ex. - if day and multiday selected for future date not for today then it can be update becouse today it will be show enable till selected date so when user click on toggle button then popup will show and selected date will be show if offline value  exist in table
                    } else if (selected_offline_tag == 2) {
                        $('#days_offline_fromdate').val(data.offline_from);
                    } else if (selected_offline_tag == 3) {
                        $('#offline_product_category_fromdate').val(data.offline_from);
                        $('#offline_product_category_tilldate').val(data.offline_to);
                    }
    ...
    $('body').on('click', '#product_online_offline_save, .product_status', function() {
        ...
        //alert('offline_tag='+offline_tag+'offline_tag_value='+offline_tag_value+'product_id='+product_id+'restaurant_id='+restaurant_id);
    ...
    $('body').on('click', '.offline_tag', function() {
        ...
        if (offline_tag == 1) {
            ...
            $('.offline_unlimited_btn_clr').addClass('btn-primary');
        ...
        if (offline_tag == 4) {
            ...
            //final value send for offline/online
            $('#offline_tag').val('4');
            $('.offline_lable_name').text('For Unlimited');
    ...
    $('body').on('change', '.category_status', function() {
        ...
        //geting cateogry offline data -------------START-------
        var selected_offline_tag = $(this).attr('selected_offline_tag');
        $('#offline_tag').val(selected_offline_tag);
        if (selected_offline_tag == 1) {
            $('.offline_hour_btn_clr').trigger('click');
        ...
        // alert(selected_offline_tag);
        $.ajax({
            url: BASE_URL + 'admin/GET_CategoryOffline_Data/', //Product detail
            data: {
        ...
        //geting cateogry offline data -------------END--------
    });
...
$('body').on('click', '.selected_variant_type_id', function() {
    ...
    $(".selected_variant_type_id:checkbox[name=variant_type_id]:checked").each(function() {
        ...
        selected_variant_type_array.push({
    ...
    $.each(selected_variant_type_array, function(index, value) {
        ...
        if (add_edit_mode == "") {
            ...
            if (exist_variant_type == 0) {
                ...
                check_edit_time_new_append.push({
...
$('body').on('click', '#submit_product_variant', function() {
    ...
    $(".selected_variant_type_id:checked").each(function() {
        ...
        if (variant_type_id != "" && variant_type_price != "" && variant_type_price != undefined) {
            ...
            array_variant_for_product.push({
                'variant_type_id': variant_type_id,
                'variant_type_price': variant_type_price
            });
        }
        count_variant_type++;
    });
    ...
    if (selected_product_id != "" && selected_variant_id != "" && ((default_variant_type_id_check != undefined && default_variant_type_id != "") || (default_variant_type_id_check == undefined && default_variant_type_id == "")) && variant_select_type_status == true && (variant_select_limit != "" || variant_select_limit == "") && array_variant_for_product.length != 0 && check_price_is_fill != "" && check_price_is_fill == true) {
        ...
        $.ajax({
            ...
            data: {
                ...
                default_variant_type_id: default_variant_type_id,
                variant_select_type: variant_select_type,
                json_of_variant_type: array_variant_type_data,
                variant_select_limit: variant_select_limit,
                add_edit_mode: add_edit_mode,
                is_variant_mandatory: is_variant_mandatory
            },
            type: 'post',
            success: function(response) {
                if (response == 1) {
...
$('body').on('click', '#edit_vairant', function() {
    ...
    if (edit_vairant_name != "" && edit_variant_id != "") {
        ...
        $.ajax({
            ...
            data: {
                edit_vairant_name: edit_vairant_name,
...
$('body').on('click', '#edit_variant_type_save', function() {
    ...
    if (edit_vairant_type_name != "" && edit_variant_type_id != "") {
        ...
        $.ajax({
            ...
            data: {
                edit_vairant_type_name: edit_vairant_type_name,
                edit_variant_type_id: edit_variant_type_id
            },
            type: 'post',
            success: function(response) {
...
$('body').on('click', '#search_order_list_data,.export_orders_csv', function() {
    ...
    var schedule_dt_to = $('#schedule_dt_to').val();
...
$('body').on('click', '.is_paid_to_restaurant_status', function() {
    ...
    $.ajax({
        ...
        data: {
            ...
            is_paid_to_restaurant_status_value: is_paid_to_restaurant_status_value
        },
        type: 'post',
        success: function(response) {
...
$('body').on('click', '.select_variant_for_order', function(e) {
    ...
    $("#select_product_variants_for_order").load(BASE_URL + 'admin/select_product_variants_for_order/' + selected_product_id + '/' + order_id);
});
        ...
        success: function(response) {
            ...
            if (response == 1) {
                ...
                /*  swal("Variant Types has been removed from the selection", {
                  ...
                  */
                $("#select_product_variants_for_order").load(BASE_URL + 'admin/select_product_variants_for_order/' + selected_product_id + '/' + order_id + '/1'); //load in modal  = select_product_with_variant_modal
...
$('body').on('click', '#check_mendetory_variant', function(e) {
    ...
    $('.order_product_change_status').each(function() {
        ...
        if($('input:radio.check_multi_limit_' + mandatory_variant_id).is(':checked')) {
         // alert("it's checked"); 
          var radio_checked = 1;
          total_mendatory_variant = 0;
        }else{
          var radio_checked = 0;
        }
        if (is_mandatory_flag != undefined && is_mandatory_flag == 1 && total_mendatory_variant > 0 && radio_checked == 0) {
            swal('Oops...', '' + variant_name + ' variant is mandatory', 'warning');
        }
    });
//alert(total_mendatory_variant+check_mendatory_is_checked+check_mendatory_is_checked);
        ...
        swal({
            ...
            if (willDelete) {
                ...
                $.ajax({
                    url: BASE_URL + 'admin/set_temporary_data_for_order_customization',
                    data: {
                        product_id_with_variant_id: product_id_with_variant_id,
                        order_product_change_status: order_product_change_status,
                        selected_order_id: order_id,
                        variant_type_price: variant_type_price,
                        product_id_for_delete: product_id_for_delete,
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response == 1) {
                    ...
                    $("#select_product_variants_for_order").load(BASE_URL + 'admin/select_product_variants_for_order/' + product_id[1] + '/' + order_id + '/1'); //load in modal  = select_product_with_variant_modal
...
$('body').on('click', '.plus_qunatity', function(e) {
    ...
    var product_unit_price = parseFloat($('.product_price_' + product_id).text());
    ...
    if (check_variant_has != undefined || check_variant_has != null) {
        ...
        $(".variant_type_price_" + product_id).each(function() {
            var variant_type_price = parseFloat($(this).text());
    ...
    product_total = parseFloat((product_total).toFixed(2));
        ...
        if (variant_is_available_total > 0) {
            ...
            if (variant_type_check != "" && variant_type_check != undefined && variant_type_check != null) {
                $(".variant_type_price_" + selected_product_id).each(function() {
                    ...
                    array_items_for_order.push({
                        'product_id': selected_product_id,
                        'product_quantity': $('.quantity_' + selected_product_id).val(),
                        'product_unit_price': product_unit_price,
                        'variant_type_id': variant_type_id,
                        'variant_type_unit_price': variant_type_price,
                ...
                array_items_for_order.push({
                    ...
                    'product_unit_price': product_unit_price,
                    'variant_type_id': 0,
                    'variant_type_unit_price': 0,
                    'variant_id': 0,
                    'check_variant_selection_if_single': check_variant_selection_if_single
                });
            }
        } else {
            var check_variant_selection_if_single = 1; // default value  in db
            array_items_for_order.push({
                'product_id': selected_product_id,
                'product_quantity': $('.quantity_' + selected_product_id).val(),
                'product_unit_price': product_unit_price,
    ...
    if (order_id != "") {
        if (subtotal_of_items_checkout != "" && subtotal_of_delivery_checkout != "" && grand_total_of_checkout != "" && delivery_total_after_promo_code_applied != "" && items_total_after_promo_code_applied != "" && item_quantity != "" && (outstanding_amount_after_customize != "" || outstanding_amount_after_customize == "") && (who_will_pay_outstanding_amount != "" || who_will_pay_outstanding_amount == "") && customer_id != "") {
            ...
            $.ajax({
                ...
                data: {
                    subtotal_of_items_checkout: subtotal_of_items_checkout,
                    subtotal_of_delivery_checkout: subtotal_of_delivery_checkout,
                    grand_total_of_checkout: grand_total_of_checkout,
                    selected_order_id: order_id,
                    delivery_total_after_promo_code_applied: delivery_total_after_promo_code_applied,
                    items_total_after_promo_code_applied: items_total_after_promo_code_applied,
                    item_quantity: item_quantity,
                    outstanding_amount_after_customize: outstanding_amount_after_customize,
                    who_will_pay_outstanding_amount: who_will_pay_outstanding_amount,
                    customer_id: customer_id
                },
                type: 'post',
                success: function(response) {
...
$('body').on('click', '#get_lat_long_by_address', function() {
    ...
    getLocation(delivery_address);

    setTimeout(function() {
        $('#updated_order_address').trigger('click');
    }, 1000);
...
$('body').on('click', '#updated_order_address', function() { 
    ...
    var old_delivery_address = $('#old_delivery_address').val();
    ...
    //alert(delivery_address+'==='+old_delivery_address);

    if (delivery_address == "" && order_type != 2) {
        $('#delivery_address').css('border-color', 'red');
        $('#unfill_delivery_address').text('Please Fill Delivery  Street Address');
    } else {
        $('#delivery_address').css('border-color', '#ccc');
    ...
    if (order_type == 2) {
        ...
        /*if(delivery_address == old_delivery_address){
            $('#delivery_address').css('border-color', 'red');
            $('#unfill_delivery_address').text('Address has not changed');
        }else*/ if (delivery_address != "" && delivery_latitude != "" && delivery_longitude != "" && delivery_street_address != "") {
            // Ajax-------SATRT------------
            $.ajax({
                ...
                data: {
                    ...
                    delivery_unit_number: delivery_unit_number,
...
$('body').on('click', '#edit_order_preparation_time_submit', function() {
    ...
    if (edit_order_preparation_time != "" && edit_order_preparation_time != 0) {
        ...
        $.ajax({
            ...
            data: {
                edit_order_preparation_time: edit_order_preparation_time,
                selected_order_id: order_id
            },
            type: 'post',
            success: function(response) {
...
$('body').on('click', '#edit_pick_up_time_submit', function() {
    ...
    if (edit_pick_up_from != "" && edit_pick_up_from != 0 && edit_pick_up_to != 0) {
        ...
        $.ajax({
            ...
            data: {
                edit_pick_up_from: edit_pick_up_from,
                edit_pick_up_to: edit_pick_up_to,
                selected_order_id: order_id,
                delivery_time: delivery_time,
                order_type: order_type,
                final_pickup_range: final_pickup_range,
                final_pickup_date_from: final_pickup_date_from
            },
            type: 'post',
            success: function(response) {
                console.log(response);
...
$('body').on('click', '#edit_order_schedule_time_submit', function() {
    ...
    if (edit_order_schedule_time != "" && edit_order_schedule_time != 0) {
        ...
        $.ajax({
            ...
            data: {
                edit_order_schedule_time: edit_order_schedule_time,
                selected_order_id: order_id
            },
            type: 'post',
            success: function(response) {
...
$('body').on('click', '#edit_order_track_link_submit', function() {
    ...
    // Client told not to make track link also mendatory. It can be empty so I removed medatory check
    ...
    // if(edit_track_link !=""){//&& lalamove_order_id !="" // Client told not to make track link also mendatory. It can be empty so I removed medatory check
    // Ajax-------SATRT------------
    $.ajax({
        url: BASE_URL + 'admin/update_order_track_link',
        data: {
            edit_track_link: edit_track_link,
            selected_order_id: order_id,
...
$(document).ready(function() {
    ...
    //Enable /Disable(active/inactive) toggle of CMS------ START------
    $('body').on('change', '.cms_status', function() {
        ...
        if (typeof checked !== 'undefined' && checked !== false) {
            //checked attribute exist means --- cms is already active and going to disable
            active_status = 2; // 2 - disable in database
        } else {
            //checked attribute not exist means --- cms is not active nd going to enable
            active_status = 1; // 1 - enable in database
        }

        // Ajax-------SATRT------------
        $.ajax({
            url: BASE_URL + 'admin/active_inactive_CMS',
            data: {
                cms_id: cms_id,
                enable_disable_status: active_status
            },
    ...
    //Enable /Disable (active/inactive)toggle of CMS------ END------

    //remove slashes when we give single quote(')----------START----------------
    ...
    //Add/Edit FAQ ----------------------START--------------------
    $('body').on('click', '.add_edit_faq', function() {
        var mode = $(this).attr('mode'); //1 = add , 2 - edit

        $('#add_edit_faq_modal').find("input").val('').end().find('textarea').text('').end();;
        if (mode == 1) {
            $('.modal_title_name').text('Add FAQ');
            $('#form_mode').val(1);
        } else if (mode == 2) {

            $('.modal_title_name').text('Edit FAQ');
            $('#form_mode').val(2);

            // we need to get edit data according to faq id ----START----
            var faq_id = $(this).attr('faq_id');
            $('#edit_faq_id').val(faq_id);
            $.ajax({
                url: BASE_URL + 'admin/GET_FAQ_Data/', //merchant detail
                data: {
                    faq_id: faq_id,
                },
                type: 'post',
                success: function(response) {
                    if (response != 0) {
                        var data = JSON.parse(response);
                        var question = stripslashes(data.question);
                        $('#question').val(question);
                        var answer = stripslashes(data.answer);
                        $('#answer').text(answer);
                    }
                },
    ...
    $('body').on('click', '.faq_delete', function() {
        ...
        var new_faq_table_url = faq_table_url.replace("table", "2"); // if action mode then it will repalce table to  2 same as  1 in enable disable  case

        var faq_id = $(this).attr('faq_id'); //input id

        swal({
            title: "Are you sure to delete this FAQ permanently?",
            text: "Once deleted, You will not be able to recover the action!",
            icon: "warning",
            buttons: true,
            dangerMode: true,
            ...
            if (willDelete) {
                ...
                //ajax-------- start-------------
                $.ajax({
                    url: BASE_URL + 'admin/DeleteFAQStatus/',
                    data: {
                        faq_id: faq_id
                    },
                    type: 'post',
                    success: function(response) {
                        if (response == 1) {
                            ...
                            setTimeout(function() {
                                $("#faq_table").load(new_faq_table_url);
                            }, 2000);
    ...
    $('body').on('click', '#edit_reservation_submit', function() {
        // alert(1211);
        var date = $('#datepicker').val();
        var no_of_people = $("#no_of_people").val();
        var dd_time_slot = $("#dd_time_slot").val();
        ...
        } else if (dd_time_slot == '') {
            $("#dd_time_slot").css('border-color', 'red');
        } else {
            $.ajax({
                url: BASE_URL + 'admin/update_reservation/',
                data: {
                    date: date,
                    no_of_people: no_of_people,
                    dd_time_slot: dd_time_slot,
                ...
                success: function(response) {
                    ...
                    if (response == 1) {
                        ...
                        swal("Sorry! Either fully booked or limited seats left to book for this slot", {
                            icon: "error",
                        });
                    }
                    else {
                        // alert("FAILED");
                        swal("Restaurant is not available for the selected date and slot", {
                            icon: "error",
                        });
                    }
                },
    ...
    //dashboard chart filter by date ---------------START-----------
    //For total sale chart by year
    $('body').on('change', '#filter_total_sale_by_year', function() {
        var selected_year = $(this).val();
        $("#total_sales_chart").load(BASE_URL+'admin/total_sales_chart/'+selected_year+'');
    });

    //For earning report
    $('body').on('click', '#filter_chart_by_date', function() {
        var from_date = $('#fromdate').val();
        var to_date = $('#todate').val();

        /*var check_given_year =  new Date(from_date).getFullYear();
         
        var current_year = new Date().getFullYear();
        if(check_given_year < 2021 || check_given_year > current_year){//give 2021 static because we data store strat from 2021
            $('#fromdate').css('border-color', 'red');
            $('#error_in_from_date').text('Please Fill Fullname');
        } else {
            $('#fromdate').css('border-color', '#ccc');
            $('#error_in_from_date').text('');
        }*/
         
        if (from_date != '' && to_date != '') {
            $("#earning_report_data").load(BASE_URL+'admin/earning_report_chart/1/'+from_date+'/'+to_date);
            $("#report_chart").load(BASE_URL+'admin/earning_report_chart/2/'+from_date+'/'+to_date);
        }
    });
    //dashboard chart filter by date ---------------END-----------
}); // End of  $(document).ready(function() {  

function edit_reservation(reservation_id) {
    // alert("QQQQ"+reservation_id);
    $.ajax({
        url: BASE_URL + 'admin/get_reservation_data_by_id/',
        data: {
            reservation_id: reservation_id,
        },
        type: 'post',
        success: function(response) {
            ...
            // alert(data.booking_date);
            $('#datepicker').val(data.booking_date);

            var no_of_people = data.no_of_people;
            $('#no_of_people').val(no_of_people);
            $('#booking_id').html(booking_id);
            // $('#time_slot').html(time_slot);
            
            // $('#time_slot_'+data.time_slot).attr('selected', 'selected');
            
            $("#dd_time_slot option[value='"+data.time_slot+"']").prop("selected",true);
            

            $('#reservation_id').val(data.id);
            $('#user_id').val(data.user_id);

            $("#edit_reservation").modal('toggle');
...
//order single page -------EDIT OUTSTANDING AMOUNT --------START------------ 
 $('body').on('click', '#edit_outstanding_amount_submit', function() {

     var edit_outstanding_amnt = $('input[name="edit_outstanding_amnt"]').val();
        if(edit_outstanding_amnt == ""){
            $('input[name="edit_outstanding_amnt"]').css('border-color','red');
            $('#validate_outstanding_amount').text('Please give amount');
        }else{
            $('input[name="edit_outstanding_amnt"]').css('border-color','#ccc');
            $('#validate_outstanding_amount').text('');
        }

      if(edit_outstanding_amnt !=""){
            // Ajax-------SATRT------------
                $.ajax({
                url: BASE_URL+'admin/update_outstanding_amount',
                data: { 
                    edit_outstanding_amnt:edit_outstanding_amnt,selected_order_id:order_id
                },
                type: 'post',
                success: function(response){
                    // console.log("response is "+response);
                    $('#updated_outstanding_amount').text(edit_outstanding_amnt);
                    $('#edit_outstanding_amount').modal('hide');
                    window.location.replace(BASE_URL+'admin/order_single/'+order_id);
                },
...
//order single page ------EDIT OUTSTANDING AMOUNT -------END------------ 